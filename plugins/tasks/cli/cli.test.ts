...
describe("zcc tasks CLI", () => {
  ...
  it("defaults create and list to the tracker project linked to CLI project context", async () => {
    ...
    expect(missing).toMatchObject({
      ...
      stdout: "",
      stderr:
        "no tracker project is linked to Zana project proj_missing; pass --project or link one with zcc tasks project update",
    });
  ...
  it("self-attaches through ZCC_THREAD_ID and lists the live thread status", async () => {
    const { bb, harness } = createFakePluginHost({
      pluginId: "tasks",
      sdk: {
        threads: {
    ...
    const previousThreadId = process.env.ZCC_THREAD_ID;
    process.env.ZCC_THREAD_ID = "thr_cli_self";
    try {
      expect(
        JSON.parse(stdout(await harness.runCli(["attach", "ATT-1", "--json"]))),
      ).toMatchObject({ task: { key: "ATT-1" }, threadId: "thr_cli_self" });
    } finally {
      if (previousThreadId === undefined) delete process.env.ZCC_THREAD_ID;
      else process.env.ZCC_THREAD_ID = previousThreadId;
    }
  ...
  it("detaches a thread with `zcc tasks detach` and lists live threads first", async () => {
    const { bb, harness } = createFakePluginHost({
      pluginId: "tasks",
      sdk: {
        threads: {
  ...
  it("attempts every --attach file after create and reports failures truthfully", async () => {
    ...
    try {
      ...
      expect(human.stdout).toContain(
        `Retry with: zcc tasks attachment add MIX-2 --file ${boomPath}`,
      );
    } finally {
      await rm(directory, { recursive: true, force: true });
      await harness.dispose();
  ...
  it("returns a friendly dispatch error when the task project is not linked", async () => {
    ...
    expect(result).toEqual({
      exitCode: 1,
      stdout: "",
      stderr: 'Task project "Unlinked CLI" is not linked to a Zana project',
    });
    ...
    expect(aliased.stderr).toBe(
      'Task project "Unlinked CLI" is not linked to a Zana project',
    );
