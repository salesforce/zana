...
const ROOT_HELP = `Usage: zcc tasks <command> [options]
...
Run zcc tasks <command> --help for command usage.`;

const PROJECT_HELP = `Usage:
  zcc tasks project create --name <name> [--prefix X] [--folder <id-or-name>] [--link-project <proj_id>] [--color <color>] [--json]
  zcc tasks project list [--json]
  zcc tasks project show <prefix-or-id> [--json]
  zcc tasks project update <prefix-or-id> [--name <name>] [--color <color>] [--folder <id-or-name> | --no-folder] [--link-project <proj_id> | --unlink-project] [--rename-prefix X] [--json]`;

const FOLDER_HELP = `Usage:
  zcc tasks folder create --name <name> [--parent <id-or-name>] [--json]
  zcc tasks folder list [--json]
  zcc tasks folder update <id-or-name> [--name <name>] [--parent <id-or-name> | --no-parent] [--json]
  zcc tasks folder delete <id-or-name> [--json]

Deleting a folder moves its projects and subfolders to the top level. No
tasks are deleted.`;

const CREATE_HELP =
  "Usage: zcc tasks create [--project <prefix-or-id>] --title <title> [--description <markdown> | --description-file <path>] [--priority <priority>] [--label <name>]... [--due YYYY-MM-DD] [--parent <key-or-id>] [--attach <path>]... [--machine <id-or-name>] [--json]";
const LIST_HELP = `Usage: zcc tasks list [--project <prefix-or-id>] [--status <status>]... [--priority <priority>]... [--label <name>]... [--active] [--search <query>] [--sort manual|priority|due] [--limit <1-${TASKS_PAGE_MAX_LIMIT}>] [--cursor <opaque>] [--json]`;
const SHOW_HELP = "Usage: zcc tasks show <key-or-id> [--json]";
const UPDATE_HELP =
  "Usage: zcc tasks update <key-or-id> [--status <status>] [--priority <priority>] [--title <title>] [--description <markdown> | --description-file <path>] [--due YYYY-MM-DD | --no-due] [--parent <key-or-id> | --no-parent] [--add-label <name>]... [--remove-label <name>]... [--machine <id-or-name>] [--json]";
const COMMENT_HELP =
  "Usage: zcc tasks comment <key-or-id> (--body <markdown> | --body-file <path>) [--author <name>] [--machine <id-or-name>] [--notify] [--json]";
const LABEL_HELP = `Usage:
  zcc tasks label create --project <prefix-or-id> --name <name> [--color <color>] [--json]
  zcc tasks label list --project <prefix-or-id> [--json]
  zcc tasks label delete --project <prefix-or-id> <name-or-id> [--json]`;
const ATTACHMENT_HELP = `Usage:
  zcc tasks attachment add <key-or-comment-id> --file <path> [--name <name>] [--machine <id-or-name>] [--json]
  zcc tasks attachment get <attachment-id> --out <path> [--machine <id-or-name>] [--json]
  zcc tasks attachment list <key> [--json]
  zcc tasks attachment remove <attachment-id> [--remove-references] [--json]
...
const PRESET_HELP = `Usage:
  zcc tasks preset list [--json]
  zcc tasks preset show <name-or-id> [--json]
  zcc tasks preset create --name <name> --provider <id> --model <id> --reasoning <level> --permission <accept-edits|auto|full> [--service-tier default|fast|none] [--environment project-default|worktree] [--base-branch <branch>] [--machine <id-or-name>] [--instructions <text>] [--json]
  zcc tasks preset update <name-or-id> [--name <name>] [--provider <id>] [--model <id>] [--reasoning <level>] [--permission <accept-edits|auto|full>] [--service-tier default|fast|none] [--environment project-default|worktree] [--base-branch <branch>] [--machine <id-or-name>] [--instructions <text>] [--json]
  zcc tasks preset delete <name-or-id> [--json]`;
const DISPATCH_HELP =
  "Usage: zcc tasks dispatch <key> --preset <name> [--instructions <extra>] [--json]";
const ATTACH_HELP =
  "Usage: zcc tasks attach <key> [--thread <thread-id>] [--json]";
const DETACH_HELP =
  "Usage: zcc tasks detach <key> [--thread <thread-id>] [--json]";
const THREADS_HELP = "Usage: zcc tasks threads <key> [--json]";
...
async function defaultProject(
  ...
  if (!ctx.projectId) {
    if (required) {
      throw new CliError(
        "missing --project and no Zana project context is available",
      );
    }
    return undefined;
  }
  ...
  if (matches.length === 0) {
    throw new CliError(
      `no tracker project is linked to Zana project ${ctx.projectId}; pass --project or link one with zcc tasks project update`,
    );
  }
  if (matches.length > 1) {
    throw new CliError(
      `multiple tracker projects are linked to Zana project ${ctx.projectId}; pass --project explicitly`,
    );
  }
  return matches[0];
}
...
async function runProject(
    ...
    const result = tasksRpcContract.createProject.output.parse(
      await domain.createProject(
        tasksRpcContract.createProject.input.parse({
          ...
          linkedBbProjectId: option(args, "link-project") ?? null,
        }),
      ),
    );
  ...
  if (action === "list") {
    assertAllowed(args, []);
    requirePositionals(args, 0, "zcc tasks project list [--json]");
    const projects = await listProjects(domain);
    const folders = tasksRpcContract.listFolders.output.parse(
      await domain.listFolders(tasksRpcContract.listFolders.input.parse(null)),
    ).folders;
  ...
  if (action === "show") {
    ...
    return detail([
      ...
      ["Color", project.color],
      ["Folder", folder?.name ?? "-"],
      ["Zana project", project.linkedBbProjectId ?? "-"],
  ...
  if (action === "update") {
    assertAllowed(
      args,
      ["name", "color", "folder", "link-project", "rename-prefix"],
      ["no-folder", "unlink-project"],
    );
    const [address] = requirePositionals(
      args,
      1,
      "zcc tasks project update <prefix-or-id> [options] [--json]",
    );
    const project = await resolveProject(domain, address!);
    const folderAddress = option(args, "folder");
    const linkedBbProjectId = option(args, "link-project");
    ...
    const changes = {
      ...
      linkedBbProjectId: args.flags.has("unlink-project")
        ? null
        : linkedBbProjectId,
    };
...
async function runFolder(
  ...
  if (action === "create") {
    ...
    requirePositionals(
      args,
      0,
      "zcc tasks folder create --name <name> [options]",
    );
  ...
  if (action === "list") {
    assertAllowed(args, []);
    requirePositionals(args, 0, "zcc tasks folder list [--json]");
  ...
  if (action === "update") {
    ...
    const [address] = requirePositionals(
      args,
      1,
      "zcc tasks folder update <id-or-name> [options] [--json]",
    );
  ...
  const stdout = args.flags.has("json")
    ...
    : [
        ...
        ...(failedAttachments.length > 0
          ? failedAttachments.map(
              (failure) =>
                `Retry with: zcc tasks attachment add ${task.key} --file ${failure.path}`,
            )
          : []),
      ].join("\n");
...
async function runLabel(domain: TasksDomain, argv: string[]): Promise<string> {
  ...
  if (action === "create") {
    ...
    requirePositionals(
      args,
      0,
      "zcc tasks label create --project <project> --name <name>",
    );
  ...
  if (action === "list") {
    ...
    requirePositionals(args, 0, "zcc tasks label list --project <project>");
    const project = await resolveProject(
      domain,
      requireOption(args, "project"),
    );
  ...
  if (action === "delete") {
    ...
    const [address] = requirePositionals(
      args,
      1,
      "zcc tasks label delete --project <project> <name-or-id>",
    );
    const project = await resolveProject(
      domain,
      requireOption(args, "project"),
...
async function runPreset(domain: TasksDomain, argv: string[]): Promise<string> {
  ...
  if (action === "list") {
    assertAllowed(args, []);
    requirePositionals(args, 0, "zcc tasks preset list [--json]");
  ...
  if (action === "update") {
    ...
    const [address] = requirePositionals(
      args,
      1,
      "zcc tasks preset update <name-or-id> [options] [--json]",
    );
    const preset = resolvePreset(await listPresets(domain), address!);
...
function resolveInvokingThreadId(
  ...
  const threadId =
    option(args, "thread") ?? process.env.ZCC_THREAD_ID ?? ctx.threadId;
  if (!threadId) {
    throw new CliError("missing --thread and ZCC_THREAD_ID is not set");
  }
  return threadId;
}
  ...
  bb.cli.register({
    ...
    commands: [
      ...
      },
      {
        ...
        usage: "zcc tasks seed-demo --yes [--json]",
      },
    ],
      ...
      try {
        ...
        switch (command) {
          case "status": {
            const args = parseArgs(rest);
            assertAllowed(args, []);
            requirePositionals(args, 0, "zcc tasks status [--json]");
          ...
          case "seed-demo": {
            ...
            requirePositionals(args, 0, "zcc tasks seed-demo --yes [--json]");
            ...
            stdout = args.flags.has("json")
              ...
              : detail([
                  ...
                  ["Zana project", result.linkedBbProjectId ?? "-"],
                ]);
            break;
          }
          default:
            throw new CliError(
              `unknown command: ${command}; run zcc tasks --help`,
            );
        }
