---
name: tasks
description: "Work on or manage records in Zana Tasks, including task keys such as ABC-12."
---

# Tasks

Use the `zcc tasks` CLI to understand the assigned task, keep its record useful,
...
1. Find and read the task before acting:
   ...
   `zcc tasks show ABC-12 --json` when the result will drive commands or code.

   For project-wide discovery, `zcc tasks list` returns at most 100 rows by
...
2. Fetch every relevant attachment before making assumptions about it:
   ...
   ```sh
   zcc tasks attachment get <attachment-id> --out <path>
   ```
...
3. Do the work. Post one substantive comment at each meaningful milestone,
   ...
   ```sh
   zcc tasks comment ABC-12 --body "Implemented the change; focused validation now passes."
   ```
...
4. Attach result artifacts that belong with the task, such as reports,
   ...
   ```sh
   zcc tasks attachment add ABC-12 --file ./report.md
   zcc tasks attachment add ABC-12 --file ./screenshot.png
   ```
...
5. Set the status to match the completion criteria. Use `done` when they are
   ...
   ```sh
   zcc tasks update ABC-12 --status in_review
   ```

   Change task hierarchy with `zcc tasks update ABC-12 --parent ABC-10`, using
   either a task key or ID for the parent. Promote a subtask to the top level
   with `zcc tasks update ABC-12 --no-parent`; the two parent flags cannot be
   combined.
...
6. Delegated threads are attached automatically. If this thread was not
   ...
   ```sh
   zcc tasks attach ABC-12
   ```

   When a thread is done with a task (hand-off, respawned replacement, or a
   predecessor that died), detach it so `zcc tasks threads ABC-12` stays
   accurate. Omit `--thread` to detach the current thread:

   ```sh
   zcc tasks detach ABC-12 --thread thr_dead_predecessor
   ```
