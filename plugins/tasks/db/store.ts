...
function validateLinkedBbProjectId(id: string | null): string | null {
  if (id !== null && !/^(?:proj_[A-Za-z0-9_-]+|[0-9a-f]{8}-(?:[0-9a-f]{4}-){3}[0-9a-f]{12})$/i.test(id)) {
    throw new Error("linkedBbProjectId must be a registered project id");
  }
  return id;
}
...
function validateThreadId(id: string | null): string | null {
  if (id !== null && !id.trim()) {
    throw new Error("threadId must be a thread id");
  }
  return id;
}
