import type { BbPluginApi, PluginMentionItem } from "../compat/server";
...
You can act on this task with the zcc tasks CLI. If you begin working on it, first run: zcc tasks attach ${task.key} (attaches THIS thread so the task shows you as working). Comment substantive updates via zcc tasks comment ${task.key} --body ... and set status via zcc tasks update ${task.key} --status ...
`;
}
