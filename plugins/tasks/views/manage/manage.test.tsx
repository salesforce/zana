...
describe("PresetDialog environment section", () => {
    ...
    return renderSlot(
      ...
      { subPath: "manage" },
      {
        rpc: {
          ...
          executionCatalog: () => ({ providers: [{ id: 'codex', displayName: 'Codex', capabilities: { supportsServiceTier: true } }], catalog: { models: [{ id: 'gpt-5.6-sol', model: 'gpt-5.6-sol', supportedReasoningEfforts: [{ reasoningEffort: 'high' }] }] } }),
          ...rpcOverrides,
        },
      },
    );
  ...
  it("hydrates a worktree preset and reconciles reasoning against its live host catalog", async () => {
    ...
    expect(slot.rpcCalls).toContainEqual({ method: 'executionCatalog', input: expect.objectContaining({ hostId: 'mach_1' }) });
    expect(slot.getByLabelText('Permissions')).toBeDefined();
  });
  ...
  it("saves the host picker's provider, model, reasoning, and tier together", async () => {
    ...
    await slot.findByRole("option", { name: "gpt-5.6-sol" });
...
describe("NewProjectDialog", () => {
  ...
  it("links the personal project from the discovered project picker", async () => {
    ...
    fireEvent.click(slot.getByLabelText("Linked Zana project"));
