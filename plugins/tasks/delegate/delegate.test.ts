...
describe("task delegation", () => {
  ...
  it("fails before spawning when the task project is not linked to bb", async () => {
    ...
    await expect(
      ...
      code: "handler_error",
      message: 'Task project "Unlinked" is not linked to a Zana project',
    });
    expect(harness.sdk.callsTo("threads.spawn")).toEqual([]);
...
describe("delegation seed prompt", () => {
  it("captures task context and the complete report-back contract", () => {
    ...
    expect(
      ...
      - Name: Tasks plugin
      - Linked Zana project: proj_tasks
      ...
      - delegation-notes.md · 01J00000000000000000000006
        Fetch with: zcc tasks attachment get 01J00000000000000000000006 --out <path>
      ...
      You are working on task TASK-1. Use the zcc tasks CLI: comment substantive updates (zcc tasks comment TASK-1 --body ...), attach result artifacts, set status when done (zcc tasks update TASK-1 --status in_review) or explain blockage in a comment. Your thread is already attached to the task.
