import type { BbPluginApi, PluginRpcHandlers } from "../compat/server";
import { z } from "zod";
import type {
  Attachment,
  Comment,
  ...
  return attachments
    .map(
      (attachment) =>
        ...
        `  Fetch with: zcc tasks attachment get ${attachment.id} --out <path>`,
    )
    .join("\n");
}
...
export function buildSeedPrompt(input: SeedPromptInput): string {
  const sections = [
    ...
    markdownSection(
      "Project context",
      `- Name: ${input.project.name}\n- Linked Zana project: ${input.project.linkedBbProjectId ?? "Not linked"}`,
    ),
    ...
    markdownSection(
      ...
      `You are working on task ${input.task.key}. Use the zcc tasks CLI: comment substantive updates (zcc tasks comment ${input.task.key} --body ...), attach result artifacts, set status when done (zcc tasks update ${input.task.key} --status in_review) or explain blockage in a comment. Your thread is already attached to the task.`,
    ),
  ];
...
function requireLinkedBbProject(project: Project): string {
  ...
  throw new DelegationError(
    "project_not_linked",
    `Task project "${project.name}" is not linked to a Zana project`,
  );
}
...
async function presetSpawnEnvironment(
  ...
  if (hostId === null) {
    throw new DelegationError(
      ...
      "Could not create a worktree because Zana has no default machine",
    );
  }
  return {
    type: "host",
