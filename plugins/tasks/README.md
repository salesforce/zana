...
Tasks is a Linear-style tracker inside Zana for planning work, delegating it to
...
Install Tasks from Zana's bundled plugins:

```sh
zcc plugin install builtin:tasks
```

The plugin adds the Tasks sidebar panel, the `zcc tasks` command, and an agent
skill that teaches workers how to report progress back to tasks.

## Quick start

Install the plugin with `zcc plugin install builtin:tasks`. Then use the `zcc tasks` CLI
to create a tracker project. Link it to the Zana project where delegated agents
will run:

```sh
zcc tasks project create \
  --name "Product" \
  --prefix PROD \
  --link-project YOUR_ZANA_PROJECT_ID
...
zcc tasks list --project PROD
zcc tasks show PROD-1
zcc tasks preset list
zcc tasks delegate PROD-1 --preset "GPT-5.6 · high"
```

When the CLI runs inside a linked Zana project, `create` and `list` infer the
...
choose or create presets under **Manage → Presets**, and type `@` in the Zana
...
`zcc tasks comment PROD-1 --body "New context" --notify`.
...
Run `zcc tasks --help` or `zcc tasks <command> --help` for exact options. Add
...
| Command                                        | Purpose                                                                                                                                    |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `zcc tasks status`                              | Show the installed Tasks plugin name and version.                                                                                          |
| `zcc tasks project create\|list\|show\|update`  | Manage tracker projects, folders, colors, prefixes, and Zana project links.                                                                  |
| `zcc tasks folder create\|list\|update\|delete` | Organize tracker projects into nested folders. Deleting a folder moves its projects and subfolders to the top level; no tasks are deleted. |
| `zcc tasks create`                              | Create a task with description, priority, labels, due date, optional parent, and file attachments (repeatable `--attach <path>`).          |
| `zcc tasks list`                                | Page/filter tasks by project, status, priority, label, active agents, or search text; supports `--sort`, `--limit`, and `--cursor`.        |
| `zcc tasks show <key-or-id>`                    | Show the complete task record, including comments, attachments, subtasks, and attached threads.                                            |
| `zcc tasks update <key-or-id>`                  | Update status, priority, title, description, due date, or labels.                                                                          |
| `zcc tasks comment <key-or-id>`                 | Add a Markdown comment from inline text or a file; optionally notify the latest responding task agent.                                     |
| `zcc tasks attachment add\|get\|list\|remove`   | Add, fetch, list, or remove attachments. Referenced attachments require `remove --remove-references`.                                      |
| `zcc tasks preset list\|create\|update\|delete` | Manage reusable agent execution presets.                                                                                                   |
| `zcc tasks delegate <key>`                      | Start and attach a new agent thread using a preset.                                                                                        |
| `zcc tasks attach <key-or-id>`                  | Attach the current Zana thread to a task when it was not delegated from Tasks.                                                               |
| `zcc tasks detach <key-or-id>`                  | Detach the current Zana thread (or `--thread <id>`) from a task, for example a dead predecessor after a respawn.                             |
| `zcc tasks threads <key>`                       | List the Zana threads attached to a task: live threads first, newest first.                                                                  |
| `zcc tasks label create\|list\|delete`          | Manage project-scoped labels.                                                                                                              |
| `zcc tasks seed-demo --yes`                     | Create sample folders, projects, labels, tasks, and comments for evaluation.                                                               |
...
Linking a Tasks project to a Zana project enables delegation. Open a task, choose
...
Delegation creates a worker thread in the linked Zana project, attaches that
...
thread with `zcc tasks attach KEY`. The inverse is `zcc tasks detach KEY
...
page and `zcc tasks threads` list live threads before completed or failed ones,
newest first.

## Task mentions

Type `@` in the Zana composer and select **Tasks** to search by task key or title.
...
action contract as context. Tasks linked to the current Zana project rank first.
