...
export function DebugLogsPanel(props: SalesforcePanelProps) {
  ...
  return <div className="sf-logs-workspace">
    <aside className="sf-log-list-pane">
      ...
      {logs.busy && !logs.data && <LoadingState />}
      <div className="sf-log-list" role="group" aria-label="Debug log list">
        ...
        {logs.data && !visible.length && <EmptyState title={records.length ? 'No matching logs' : 'No debug logs returned'}>{records.length ? 'Try another operation or status.' : 'Generate a log in the org and refresh.'}</EmptyState>}
      </div>
    </aside>
    <section className="sf-log-reader" aria-label="Log details">
      ...
      </> : !log.busy && !log.error && <EmptyState title="Select a log">Choose an execution to inspect its output and find errors.</EmptyState>}
    </section>
  </div>;
}
