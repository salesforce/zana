...
| `posthog-analytics/` | Official (`autoInstall: false`) — usage analytics: agent lifecycle events plus optional content-free UI-click ids. **On by default**, preconfigured to a shared community PostHog project (bring your own key to redirect it, or turn it off); never sends prompt/response content, labels, or input values. |
