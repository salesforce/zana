...
${Bt}`,document.head.appendChild(t)}Gs();var Vs={height:"100%",minHeight:0,display:"flex",flexDirection:"column"};function js(){let e=Q(()=>Vo($o),[]);return a("div",{style:Vs,children:a(Mt,{host:e})})}function $s(){let[e,t]=u(null);return q(()=>{let s=!0,r=async()=>{try{let d=await ra($o,"badge");if(!s)return;let i=typeof d?.count=="number"&&d.count>0?d.count:null;t(i)}catch{s&&t(null)}};r(),Nt(()=>{r()});let l=window.setInterval(()=>{r()},3e4);return()=>{s=!1,window.clearInterval(l),Nt(void 0)}},[]),e==null?null:a("span",{className:"nav-badge",children:e})}var $f=Ut(e=>{e.slots.navPanel({id:"main",title:"PR Monitor",icon:"GitPullRequest",component:js,experimental_sidebarAccessory:$s}),e.slots.commandPaletteAction({id:"open",title:"Open PR Monitor",run:t=>{t.toPluginPanel("main")}})});export{$f as default,Gs as injectStyles};
