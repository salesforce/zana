Give an agent the API keys it needs without pasting them into chat. The agent asks, you type the values into a masked form, and ZCC writes them to a dotenv file on the thread's host.
...
```
ZCC secret request OPENAI_API_KEY --purpose "Configure the server" --describe OPENAI_API_KEY "OpenAI key" --write-env .env.local
```
...
The command must run from a ZCC thread with a live host.
