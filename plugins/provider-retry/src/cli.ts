import type { PluginCliContext, ZccPluginApi } from "@zana-ai/zcc-plugin-sdk/server";
import type { ProviderRetryView } from "./contract.js";
import type { ProviderRetryService } from "./service.js";

function requestedThreadId(argv: string[], context: PluginCliContext): string | null {
    ...
    commands: [
      {
        ...
        usage: "zcc provider-retry status [thread-id] [--json]",
      },
      {
        name: "cancel",
        summary: "Cancel a pending automatic provider retry",
        usage: "zcc provider-retry cancel <thread-id> [--json]",
      },
      {
        name: "retry",
        summary: "Manually continue a provider-limited turn",
        usage: "zcc provider-retry retry <thread-id> [--json]",
      },
    ],
    async run(argv, context) {
      ...
      if (command !== "status" && command !== "cancel" && command !== "retry") {
        return {
          exitCode: 2,
          stderr:
            "Usage: zcc provider-retry <status|cancel|retry> [thread-id] [--json]\n",
        };
      }
      ...
      if (command === "retry") {
        if (threadId === null) {
          return {
            exitCode: 2,
            stderr:
              "A thread id is required: zcc provider-retry retry <thread-id>\n",
          };
        }
      ...
      if (command === "cancel") {
        if (threadId === null) {
          return {
            exitCode: 2,
            stderr:
              "A thread id is required: zcc provider-retry cancel <thread-id>\n",
          };
        }
