...
const { parsePanelParams } = await import('./app.tsx');
...
function stubHostRpc(handler: (method: string, input: unknown) => unknown): ReturnType<typeof vi.fn> {
  const callRpc = vi.fn(async (_pluginId: string, method: string, input?: unknown) =>
...
describe('registrations', () => {
  it('registers the reply message action and the side-chat panel action', () => {
    expect(app.messageActions.map((action) => action.id)).toEqual(['reply-in-side-chat']);
    expect(app.threadPanelActions.map((action) => action.id)).toEqual(['side-chat']);
  });
});
...
describe('parsePanelParams', () => {
  it('narrows persisted params and rejects malformed values', () => {
    ...
    expect(parsePanelParams({ threadId: 'thr_fork', sourceThreadId: 'thr_src' })).toEqual({
      threadId: 'thr_fork',
      sourceThreadId: 'thr_src',
      sourceMessageText: '',
      sourceSeqEnd: null
...
describe('reply-in-side-chat message action', () => {
  it('creates the fork then opens the panel tab pointing at it', async () => {
    const callRpc = stubHostRpc((method) => {
    ...
    expect(callRpc).toHaveBeenCalledWith('side-chat', 'createSideChat', {
      sourceThreadId: 'thr_src',
      sourceSeqEnd: 42,
...
describe('SideChatPanel', () => {
  ...
  it('renders ThreadChat with the ReplyingTo header and send-to-main action', () => {
    const slot = renderSlot(
      ...
      { pluginId: 'side-chat', threadId: 'thr_src', params },
      { rpc: {} }
    );

    const chat = slot.getByTestId('plugin-thread-chat');
    ...
    expect(chat.getAttribute('data-include-plugin-message-actions')).toBe('false');

    const leading = slot.getByTestId('plugin-thread-chat-leading-content');
    expect(leading.textContent).toContain('Replying to');
    const markdown = leading.querySelector("[data-testid='plugin-markdown']");
    expect(markdown?.textContent).toContain('**anchor**');

    const action = slot.getByTestId('plugin-thread-chat-action-send-to-main');
  ...
  it('send-to-main queues the message text on the source thread', async () => {
    const slot = renderSlot(
      app.threadPanelActions[0]!,
      { pluginId: 'side-chat', threadId: 'thr_src', params },
      { rpc: { sendToMain: () => ({ ok: true }) } }
    );

    fireEvent.click(slot.getByTestId('plugin-thread-chat-action-send-to-main'));
  ...
  it('reports a missing thread reference for malformed params', () => {
    const slot = renderSlot(
      app.threadPanelActions[0]!,
      { pluginId: 'side-chat', threadId: 'thr_src', params: { bogus: true } },
      { rpc: {} }
    );
    expect(slot.getByRole('alert').textContent).toContain('missing its thread reference');
  });
});
