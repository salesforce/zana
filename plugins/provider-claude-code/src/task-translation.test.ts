...
describe("claude-code background-task occupancy helpers", () => {
  function taskMap(
    rows: Array<{
      terminal: boolean;
    ...
    return new Map(
      rows.map((row, index) => [
        String(index),
        {
          ...
          skipTranscript: row.skipTranscript ?? false,
  ...
  it("treats any non-terminal task as occupancy that must pin translator state", () => {
    expect(hasOpenClaudeBackgroundTasks(taskMap([]))).toBe(false);
    expect(
      hasOpenClaudeBackgroundTasks(
        taskMap([{ terminal: true, taskType: "local_workflow" }]),
      ),
    ).toBe(false);
    expect(
      hasOpenClaudeBackgroundTasks(
        taskMap([{ terminal: false, taskType: "local_workflow" }]),
      ),
    ).toBe(true);
    expect(
      hasOpenClaudeBackgroundTasks(
        taskMap([{ terminal: false, taskType: "local_bash" }]),
      ),
    ).toBe(true);
  });

  it("does not let workflows or detached shells block turn completion", () => {
    expect(
      hasCompletionBlockingClaudeTasks(
        taskMap([{ terminal: false, taskType: "local_subagent" }]),
      ),
    ).toBe(true);
    expect(
      hasCompletionBlockingClaudeTasks(
