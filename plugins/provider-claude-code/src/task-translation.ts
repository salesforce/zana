...
/**
 * Whether any Claude background task is still open. Broader than
 * {@link hasCompletionBlockingClaudeTasks}: workflows and detached shells
 * must not hold the bb turn open, but they still pin translator state so
 * LRU eviction cannot orphan their timeline items.
 */
export function hasOpenClaudeBackgroundTasks(
  tasks: ClaudeTaskMap,
): boolean {
