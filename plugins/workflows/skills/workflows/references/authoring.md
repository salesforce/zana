...
- `agent(prompt: string, opts?)`: spawn a ZCC worker. Without `schema`, returns
  its final text as a string. With `schema` (a JSON Schema), the worker is forced
  to call `zcc_workflow_result` and `agent()` returns the validated value — no
...
- `args`: the value passed as `zcc_workflow_run`'s `args` input, verbatim. Pass
...
Workers spawned by `agent()` are normal ZCC threads and retain the tools and
workspace access allowed by their ZCC permission mode. The QuickJS script itself
...
ZCC validates the tuple against the live provider/model catalog immediately
...
That worker receives only the `zcc_workflow_result` plugin tool. It MUST call the
tool exactly once at the end of its response with `{ value: ... }` to provide
the structured output. ZCC validates the value with Ajv. The initial invalid
