...
`zcc_workflow_run` and `zcc workflows validate` accept exactly one source mode:
...
- `name`: a lowercase kebab-case name resolved as
  `.zcc/workflows/<name>.js` in the current project workspace.
...
`zcc_workflow_run` also accepts optional JSON `args` and optional `resumeRunId`.
It returns a durable run ID immediately. Use the compact `zcc workflows status`
summary, paged `zcc workflows history`, `zcc workflows list`, and
`zcc workflows stop` afterward. Completion is sent back as an agent-only input:
...
```bash
run=<run-id>
mkdir -p "$ZCC_THREAD_STORAGE/workflows"
zcc workflows history "$run" --cursor 0 --limit 100 \
  > "$ZCC_THREAD_STORAGE/workflows/$run.jsonl"
jq -c 'select(.type == "page")' "$ZCC_THREAD_STORAGE/workflows/$run.jsonl"
```
...
```bash
zcc workflows validate --script '<javascript>'
zcc workflows validate --file .zcc/workflows/review-change.js
zcc workflows validate --name review-change
zcc workflows run --script '<javascript>' --args '<json>'
zcc workflows run --file .zcc/workflows/review-change.js --resume <run-id>
zcc workflows run --name review-change
zcc workflows status <run-id>
zcc workflows history <run-id> --cursor 0 --limit 100
zcc workflows list --limit 20
zcc workflows stop <run-id>
zcc workflows validate --script '<javascript>'
```
