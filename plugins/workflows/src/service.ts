...
export function formatWorkflowNotification(
  ...
  const prefix = `[ZCC workflow finished · ${run.id}]\n\nRun ${run.id} (${run.name}) ${run.status}.\n`;
  const suffix = `\nRun \`zcc workflows status ${run.id}\` for authoritative details.`;
  const detail =
  ...
  return utf8Prefix(
    `[ZCC workflow ${run.id}] ${run.status} — run zcc workflows status ${run.id}`,
    maximumBytes,
  );
}
...
export function createWorkflowService(
  ...
  async function resolveOrigin(input: StartWorkflowInput) {
    ...
    if (thread === null) {
      throw new Error("Workflow origin thread was not found");
    }
  ...
  async function validateSelection(
    ...
    const allowedModes = provider.capabilities?.permissionModes;
    if (allowedModes && !allowedModes.includes(permissionMode)) {
      throw new Error(
    ...
    while (true) {
      try {
        ...
        const child = await bb.sdk.threads.spawn({
          ...
          environment: { kind: "reuse", environmentId: run.environmentId },
          prompt: childPrompt(run, prompt, options),
          title: options.title ?? `${run.name} · ${callIndex + 1}`,
          providerId: selection.providerId,
          model: selection.model,
          reasoningLevel: selection.reasoningLevel,
          permissionMode: selection.permissionMode as
            | "accept-edits"
            | "auto"
            | "full",
          visibility: "hidden",
        ...
        try {
          ...
          throwIfCancelled(signal);
          if (current === null) {
            failThreadCall(child.id, "Workflow worker was deleted");
    ...
    if (options.outputSchema !== null) {
      ...
      try {
        await bb.sdk.threads.send({
          threadId,
          mode: "auto",
          prompt: `Structured result missing or invalid (${detail}). Call zcc_workflow_result with one value matching the required schema. Corrective turn ${attempts} of ${MAX_REPAIR_ATTEMPTS}.`,
        });
        return;
      } catch (error) {
      ...
      instructions:
        call.status !== "running"
          ...
          : options.outputSchema === null
            ? null
            : `You are a ZCC workflow worker. Submit your final value with zcc_workflow_result. Required schema: ${JSON.stringify(options.outputSchema)}`,
    };
  }
  ...
  async function notify(run: WorkflowRunRow): Promise<void> {
    ...
    try {
      await bb.sdk.threads.send({
        ...
        prompt: formatWorkflowNotification(
          latest,
