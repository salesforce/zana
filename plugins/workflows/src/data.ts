...
export function applyWorkflowMigrations(database: {
  migrate(statements: readonly string[]): void;
}): void {
  try {
    // Hosts hash migrate() statements by index. Passing them one-at-a-time
    // records each as index 0 and then throws "does not match the recorded statement".
    database.migrate(migrations);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    // Hosts that re-exec every migrate() statement throw on ALTER ADD COLUMN after the first load.
