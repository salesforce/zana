...
function requireContext(ctx: PluginCliContext): {
  ...
  const projectId =
    ctx.projectId ?? process.env.ZCC_PROJECT_ID ?? process.env.BB_PROJECT_ID;
  const threadId =
    ctx.threadId ??
    process.env.ZCC_THREAD_ID ??
    process.env.ZCC_SESSION_ID ??
    process.env.BB_THREAD_ID;
  if (projectId === undefined || threadId === undefined) {
    throw new Error(
      "This command must run inside a ZCC project thread (set ZCC_PROJECT_ID and ZCC_THREAD_ID)",
    );
  }
...
function statusSummary(page: WorkflowRunInspectionPage) {
  ...
  return {
    ...
    history: {
      format: "jsonl",
      pageUsage: `zcc workflows history ${run.id} --cursor 0 --limit ${DEFAULT_HISTORY_LIMIT}`,
      fileUsage: `mkdir -p "$ZCC_THREAD_STORAGE/workflows" && zcc workflows history ${run.id} --cursor 0 --limit ${DEFAULT_HISTORY_LIMIT} > "$ZCC_THREAD_STORAGE/workflows/${run.id}.jsonl"`,
    },
  };
}
...
export function registerWorkflowCli(
  ...
  bb.cli.register({
    name: "workflows",
    summary: "Run and inspect durable ZCC workflows",
    commands: [
      {
        name: "run",
        summary: "Start a workflow and return immediately",
        usage:
          "zcc workflows run (--script '<javascript>'|--file <path>|--name <name>) [--args '<json>'] [--resume <run-id>]",
      },
      {
        name: "validate",
        summary: "Validate workflow source and literal model selections",
        usage:
          "zcc workflows validate (--script '<javascript>'|--file <path>|--name <name>)",
      },
      {
        name: "status",
        summary: "Show a compact workflow run summary",
        usage: "zcc workflows status <run-id>",
      },
      {
        name: "history",
        summary: "Read one JSONL page of workflow run and call history",
        usage:
          "zcc workflows history <run-id> [--cursor <call-index>] [--limit <1-100>]",
      },
      {
        name: "list",
        summary: "List recent project workflow runs",
        usage: "zcc workflows list [--limit <1-50>]",
      },
      {
        name: "stop",
        summary: "Cancel a workflow run",
        usage: "zcc workflows stop <run-id>",
      },
    ],
    async run(argv, ctx) {
      try {
        ...
        return failure(
          "Usage: zcc workflows <run|validate|status|history|list|stop> [options]",
        );
      } catch (error) {
