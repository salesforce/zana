...
import type { ZccPluginApi, PluginRpcHandlers } from "@zana-ai/zcc-plugin-sdk";
import { z } from "zod";
import {
  hostContract,
  rpcContract,
...
export default async function browserAutomationPlugin(zcc: ZccPluginApi) {
  const host = zcc.host.experimental_client({ contract: hostContract });
  const desktop = zcc.sdk.experimental_desktopBrowsers;
  ...
  async function owned(threadId: string, sessionId: string) {
    const record = recordSchema.parse(
      await zcc.storage.kv.get(keyFor(threadId, sessionId)),
    );
  ...
  for (const key of await zcc.storage.kv.list("sessions/")) {
    const parsed = recordSchema.safeParse(await zcc.storage.kv.get(key));
  ...
  async function open(
    ...
    await zcc.sdk.threads.get({ threadId: input.threadId });
    if (active.size >= 64)
      throw new Error(
      ...
      async list({ threadId }) {
        const records = await Promise.all(
          (
            ...
            .map(
              async (key) =>
                recordSchema.parse(await zcc.storage.kv.get(key)).session,
            ),
        );
        return records;
      },
  ...
  zcc.rpc.register(rpcContract, handlers(lifecycle.signal));
  function dispatch(
    ...
    async run(argv, context) {
      try {
        ...
        if (parsed.scriptFile) {
          ...
          const file = await zcc.sdk.files.read({
  ...
  for (const event of [
    ...
    zcc.events.on(event, async (payload) => {
      const id = payload.thread?.id ?? payload.threadId;
      if (!id) return;
      threadLifecycles.get(id)?.abort();
      threadLifecycles.delete(id);
      const results = await Promise.allSettled(
        [...active.values()]
          .filter((record) => record.session.threadId === id)
          .map((record) => finish(record, "closed")),
      );
      if (results.some((result) => result.status === "rejected")) {
        zcc.log.warn(
          `Browser Automation cleanup for thread ${id} needs its host to reconnect`,
        );
      }
    });
  }
