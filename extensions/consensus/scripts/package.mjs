#!/usr/bin/env node
/**
 * Package the built consensus extension into its committed, runnable artifact
 * dir and (best-effort) seed it into the dev install dir so a fresh
 * `npm run dev` shows consensus through the disk pipeline. Mirrors
 * extensions/gus/scripts/package.mjs.
 *
 * Sources:  extensions/consensus/extension.json + dist/{main.mjs,renderer.js}
 * Targets:
 *   1. examples/extensions/consensus/  — COMMITTED runnable artifact (the
 *      canonical shipped form, seeded on boot by seedBundledExtensions).
 *   2. ~/.zcc/extensions/consensus/    — the dev/runtime install dir the
...
async function main() {
  ...
  const examplesDir = join(repoRoot, 'examples', 'extensions', 'consensus');
  ...
  try {
    await copyInto(join(installRoot, 'consensus'), build);
    console.log(`seeded   → ${join(installRoot, 'consensus')}`);
  } catch (err) {
