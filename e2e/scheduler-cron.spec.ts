 ...
 * DOM (not IPC): open New schedule, switch the cadence toggle to Cron, type an
 * expression, and assert the live-validation + next-fires preview react as the
 * user types. Then save through the real Create button and confirm the persisted
 * schedule carries a `cron` cadence (not `every`).
 */
...
test('scheduler: the Cron cadence UI validates, previews, and saves a cron schedule', async ({
  ...
  try {
    ...
    // Open the Scheduler and its dedicated new-schedule page.
    ...
    const editor = window.locator('[data-testid="schedule-editor"]');
    await expect(editor).toBeVisible({ timeout: 15_000 });

    // Name + project so the form can eventually save.
    await editor.locator('#sched-name').fill('E2E cron schedule');
    // Pick our project through the portal-rendered project list.
    const projectPicker = editor.getByRole('button', { name: 'Project' });
    ...
    await expect(editor.locator('#sched-cron')).toHaveCount(0);

    // Switch to Cron. The toggle is a tablist with Interval / Cron buttons.
    await editor.locator('[role="tab"]', { hasText: 'Cron' }).click();
    const cronInput = editor.locator('#sched-cron');
    await expect(cronInput).toBeVisible({ timeout: 5_000 });

    // It seeds with a valid default ("0 9 * * 1-5") → live preview shows next fires.
    await expect(editor.locator('.scheduler-interval-feedback--ok')).toContainText('Next:', {
      timeout: 5_000,
    });
    ...
    await expect(editor.locator('.scheduler-interval-feedback--err')).toBeVisible();
    const createBtn = editor.locator('button', { hasText: 'Create schedule' });
    ...
    await expect(window.locator('#sched-name')).toHaveValue('E2E cron schedule');
