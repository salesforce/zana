...
export function createDesktopBrowserBrokerClient(args: {
  ...
  async function connect(): Promise<void> {
    ...
    try {
      ...
      if (registryServerOrigin !== null && registryServerOrigin !== serverOrigin) {
      ...
      connection.on('message', (data: { toString(): string }, binary: boolean) => {
        ...
        requests = requests.then(async () => {
          if (connection.readyState !== WebSocket.OPEN) return;
          try {
            const result = desktopBrowserResultSchemas[parsed.command.type].parse(
              await args.broker.execute(parsed.command)
            );
