...
const mocks = vi.hoisted(() => ({
  ...
  validate: vi.fn(), transcript: vi.fn(), provider: vi.fn(), create: vi.fn(), resume: vi.fn(), claude: vi.fn(), opencode: vi.fn()
}));
...
vi.mock('./ctx.js', () => ({ ctx: {
  ...
  conversationHistory: {
    start: mocks.start, get: mocks.get, scope: mocks.scope, query: mocks.query, refresh: mocks.refresh,
    release: mocks.release, find: mocks.find, validate: mocks.validate, transcript: mocks.transcript, provider: mocks.provider
  },
  createInteractiveTerminal: mocks.create
} }));
...
beforeEach(() => {
  ...
  mocks.provider.mockImplementation(() => ({ resume: (id: string) => mocks.resume(id) }));
...
it('validates native membership before exact resume and uses the row owner, never renderer paths', async () => {
  ...
  expect(mocks.validate).toHaveBeenCalledWith(expect.objectContaining({ source: 'codex', nativeConversationId: 'exact-id', projectId: 'p' }));
...
it('revalidates OpenCode sessions and reads its transcript', async () => {
  ...
  await call('resume', 's', 'opaque');
  expect(mocks.validate).toHaveBeenCalledWith(expect.objectContaining({ source: 'opencode', nativeConversationId: 'exact-id' }));
  expect(mocks.create).toHaveBeenCalledTimes(1);
  mocks.validate.mockResolvedValueOnce(false); expect(await call('resume', 's', 'opaque')).toMatchObject({ code: 'NOT_FOUND' }); expect(mocks.create).toHaveBeenCalledTimes(1);
});
