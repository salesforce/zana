...
export function createDesktopBrowserBroker(args: {
  ...
  function publish(instance: InstanceEntry, threadId: string): void {
    ...
    for (const tab of tabs) {
      instance.window.webContents.send(IPC.browser.control, {
        tabId: tab.tabId,
        threadId,
        control: tab.control,
      });
  ...
  function reveal(
    ...
    instance.window.webContents.send(IPC.browser.reveal, {
      tabId,
      threadId,
      desktopTarget: {
        hostId,
  ...
  return {
    registerWindow(window: BrokerWindow) {
      ...
      const descriptor = {
        ...
        label: `ZCC window ${window.webContents.id}`,
      };
    ...
    getTarget(webContentsId: number): DesktopBrowserTarget | null {
      const instance = instanceForWindow(webContentsId);
    ...
    getControl(
      ...
      tabId: string,
    ): DesktopBrowserControlState | null {
      const instance = instanceForWindow(webContentsId);
      const tab =
        instance &&
        args.manager
