...
describe('spawn-child env isolation (finding A)', () => {
  ...
  describe('buildChildEnv', () => {
    it('includes PATH/HOME but excludes a sentinel secret', () => {
    ...
    it('copies only allowlisted keys that are actually set (no empty placeholders)', () => {
      const env = buildChildEnv({ PATH: '/bin' });
      expect(Object.keys(env)).toEqual(['PATH']);
    });

    it('forwards Electron bootstrap vars only when present', () => {
