...
describe('startup migration bootstrap integration', () => {
  it('routes app readiness through migration before normal bootstrap', () => {
    ...
    expect(source).toContain('void reconcileOpenCodeStartupRouting(');
    expect(source).toMatch(/try \{[\s\S]*void reconcileOpenCodeStartupRouting\([\s\S]*logMainError\('reconcileOpenCodeStartupRouting'/);
    expect(source).toContain("if (result.outcome === 'cas-give-up')");
  });
