...
const tab = {
  ...
  url: 'about:blank',
  title: '',
  profile: { kind: 'personal' as const },
...
async function setup(timeoutMs = 1000) {
  const dataDir = await mkdtemp(join(tmpdir(), 'zcc-browser-broker-'));
  directories.push(dataDir);
  const onChanged = vi.fn();
  const broker = await startDesktopBrowserBroker({
    dataDir,
    hostId: 'host-1',
    serverUrl: 'https://zcc.example',
    onChanged,
...
async function registered(broker: DesktopBrowserBroker) {
  await vi.waitFor(async () => {
    expect(
      (await broker.request({ type: 'desktop.browser.list_instances' })).instances
    ).toHaveLength(1);
  });
}
...
describe('desktop browser broker', () => {
  ...
  it('rejects unauthenticated, renderer-origin, wrong-server, and offline clients', async () => {
    ...
    expect(((await once(offline, 'error'))[0] as Error).message).toContain('401');
  });
