...
export interface InteractiveRequestRegistryOptions {
  onRegistrationFailure?: (failure: InteractiveRequestRegistrationFailure) => void;
...
function buildInteractiveRequestKey(
  request: Pick<PendingInteractionCreate, 'providerId' | 'providerRequestId' | 'providerThreadId' | 'threadId'>
): string {
  return [request.threadId, request.providerId, request.providerThreadId, request.providerRequestId].join('\0');
}
...
export class InteractiveRequestRegistry {
  private readonly deliveredTombstones = new Map<string, DeliveredInteractiveRequestTombstone>();
  ...
  async registerAndWait(request: PendingInteractionCreate): Promise<PendingInteractionResolution> {
    ...
    try {
      ...
      if (response.outcome === 'rejected') {
        ...
        entry.reject(new InteractiveRequestRegistryError('interactive_request_rejected', response.reason));
        return promise;
      }
      entry.interactionId = response.interactionId;
      if (response.status !== 'pending' && response.status !== 'resolving') {
        this.pendingEntries.delete(key);
        entry.reject(new Error(`Pending interaction ${response.interactionId} is already ${response.status}`));
      }
    } catch (error) {
      this.pendingEntries.delete(key);
      const registrationError = normalizeCaughtError(error);
      this.options.onRegistrationFailure?.({ error: registrationError, request });
  ...
  resolve(request: InteractiveResolveCommandInput): void {
    ...
    if (this.deliveredTombstones.has(tombstoneKey)) return;
    ...
    if (entry.interactionId !== null && entry.interactionId !== request.interactionId) {
      throw new InteractiveRequestRegistryError(
  ...
  interruptThreads(args: InterruptInteractiveThreadsArgs): void {
    ...
    for (const [key, entry] of this.pendingEntries) {
      if (entry.request.providerId !== args.providerId || !threadIds.has(entry.request.threadId)) {
        continue;
      }
      this.pendingEntries.delete(key);
