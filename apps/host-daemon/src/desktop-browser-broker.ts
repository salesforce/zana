...
export async function startDesktopBrowserBroker(options: {
  ...
  server.on('upgrade', (request, socket, head) => {
    ...
    websocketServer.handleUpgrade(request, socket, head, (websocket) => {
      ...
      websocket.on('message', (data: Buffer, isBinary: boolean) => {
        try {
          if (!connected || isBinary) throw new Error('Invalid desktop message');
          const value: unknown = JSON.parse(data.toString());
          const registration = desktopBrowserRegistrationSchema.safeParse(value);
          ...
          if (!request) return;
          if (request.peer !== peer) throw new Error('Desktop response owner mismatch');
          ...
          const result = desktopBrowserResultSchemas[request.command.type].safeParse(response.result);
          if (!result.success) {
          ...
          if (
            ...
            && (
              ('tabs' in result.data
                && result.data.tabs.some((tab) => tab.threadId !== command.threadId))
              || ('tab' in result.data
                && (result.data.tab.threadId !== command.threadId
                  || ('tabId' in command && result.data.tab.tabId !== command.tabId)))
            )
          ) {
  ...
  const descriptorPath = join(options.dataDir, DESKTOP_BROWSER_BROKER_DESCRIPTOR_FILE);
  ...
  async function request(
    ...
    if (!peer) {
      throw new Error('Desktop browser instance is unavailable or generation is stale');
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        peer.socket.send(JSON.stringify({ type: 'request', requestId, command }));
      } catch {
        disconnect(peer);
      }
    });
