...
import { LEAST_CAPABLE } from '@zana-ai/zcc-domain/launch-provider';
import type { AppConfig, LaunchProfileId, Persona, ProjectSettings, ProjectRemote } from '@zana-ai/zcc-domain/product';
...
describe('ClaudeCodeProvider.buildRemoteCommand', () => {
  ...
  it('forwards MCP: inline --mcp-config with the loopback URL + inbox allowlist + guidance', () => {
    ...
    expect(cmd).toContain('mcp__zcc-inbox__schedule_list');
    ...
    // Mutations (run-now / set-enabled) are not pre-approved.
    expect(cmd).not.toContain('mcp__zcc-inbox__schedule_run_now');
    expect(cmd).not.toContain('mcp__zcc-inbox__schedule_set_enabled');
