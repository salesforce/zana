...
function parseCookieNameValue(setCookie: string): string | null {
  ...
  if (!trimmedName || !isAllowedCloudflareCookieName(trimmedName)) return null;
...
export function storeChatGptCloudflareCookies(url: string, headers: Headers): void {
  ...
  for (const cookie of splitSetCookieHeader(setCookie)) {
    ...
    const [name] = nameValue.split('=', 1);
    if (name) cloudflareCookiesByName.set(name, nameValue);
  }
}

export function getChatGptCloudflareCookieHeader(url: string): string | null {
  if (!isAllowedChatGptUrl(url) || cloudflareCookiesByName.size === 0) return null;
