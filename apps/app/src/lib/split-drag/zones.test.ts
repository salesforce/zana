...
describe('decideThreadDrop', () => {
  it('passes an edge zone through as a split when there is room', () => {
    expect(decideThreadDrop({ zone: 'left', threadAlreadyOpen: false, atMaxPanes: false })).toEqual({
      zone: 'left',
      label: 'Split left'
    });
  });
  ...
  it('coerces to focus-pane when the thread is already open, overriding the cap', () => {
    expect(decideThreadDrop({ zone: 'right', threadAlreadyOpen: true, atMaxPanes: true })).toEqual({
      zone: 'center',
      label: 'Already open — focus pane'
    });
  });
});
...
describe('shouldEngageSidebarSplitDrag', () => {
  ...
  it('does not engage while the pointer is still inside the sidebar', () => {
    expect(shouldEngageSidebarSplitDrag({ ...base, x: 200, y: 340 })).toBe(false);
  });
