import type { SplitSide } from '../split-layout/types.js';
...
export function pickZone(rect: Rect, clientX: number, clientY: number): SplitZone {
  ...
  if (px < EDGE_X_FRACTION) return 'left';
  if (px > 1 - EDGE_X_FRACTION) return 'right';
  if (py < EDGE_Y_FRACTION) return 'top';
  if (py > 1 - EDGE_Y_FRACTION) return 'bottom';
  return 'center';
}
...
export function decideThreadDrop({
  ...
  return {
    zone,
    label: zone === 'center' ? replaceLabel : `Split ${zone}`
  };
}
