...
function replacePaneNode(node: LayoutNode, paneId: string, replacement: LayoutNode): LayoutNode {
  ...
  return {
    ...node,
    children: node.children.map((child) => replacePaneNode(child, paneId, replacement))
  };
}
...
function insertPane(root: LayoutNode, targetPaneId: string, side: SplitSide, pane: PaneNode): LayoutNode {
  if (root.type === 'pane') {
  ...
  if (root.dir === direction && directTargetIndex !== -1) {
    const insertionIndex =
      side === 'left' || side === 'top' ? directTargetIndex : directTargetIndex + 1;
    const children = [...root.children];
    children.splice(insertionIndex, 0, pane);
    return { ...root, children, sizes: equalSizes(children.length) };
  }

  const targetChildIndex = root.children.findIndex((child) => findPane(child, targetPaneId) !== null);
  ...
  return {
    ...
    children: root.children.map((child, index) =>
      index === targetChildIndex ? insertPane(child, targetPaneId, side, pane) : child
    )
  };
}
...
export function splitPane(
  ...
  if (countPanes(layout.root) >= MAX_PANES || findPane(layout.root, targetPaneId) === null) {
    return layout;
  }
  const pane: PaneNode = {
    type: 'pane',
...
function detachPane(node: LayoutNode, paneId: string): DetachResult {
  if (node.type === 'pane') {
    return node.paneId === paneId ? { node: null, detached: node } : { node, detached: null };
  }
  ...
  for (let index = 0; index < node.children.length; index += 1) {
    ...
    if (result.node === null) {
      const children = node.children.filter((_candidate, childIndex) => childIndex !== index);
...
export function swapPanes(layout: SplitLayout, paneId: string, targetPaneId: string): SplitLayout {
...
function normalizeSizes(sizes: readonly number[], childCount: number): number[] {
  if (sizes.length !== childCount || sizes.some((size) => !Number.isFinite(size) || size <= 0)) {
  ...
  const normalized = sizes.map((size) => Math.min(MAX_SIZE, Math.max(MIN_SIZE, size / total)));
  ...
  for (let pass = 0; pass < childCount + 1; pass += 1) {
    ...
    const adjustable = normalized
      ...
      .filter(({ size }) => (difference > 0 ? size < MAX_SIZE : size > MIN_SIZE));
    ...
    for (const { index } of adjustable) {
      ...
      if (size !== undefined) {
        normalized[index] = Math.min(MAX_SIZE, Math.max(MIN_SIZE, size + adjustment));
      }
    }
  }
...
function updateSplitAtPath(
  ...
  const child = updateSplitAtPath(node.children[childIndex], splitPath, pathIndex + 1, update);
  if (child === null) {
    return null;
  }
  return {
    ...node,
    children: node.children.map((candidate, index) => (index === childIndex ? child : candidate))
  };
}
...
export function normalize(layout: SplitLayout): SplitLayout {
  ...
  const focusedPaneId = panes.some((pane) => pane.paneId === layout.focusedPaneId)
    ? layout.focusedPaneId
