...
function parseBareAddressHost(input: string): string | null {
  const bareAddressMatch = input.match(BARE_ADDRESS_PATTERN);
  ...
  const bracketedHostMatch = authority.match(BRACKETED_HOST_AUTHORITY_PATTERN);
  if (bracketedHostMatch !== null) {
    return (bracketedHostMatch[1] ?? '').toLowerCase();
  }

  const hostMatch = authority.match(HOST_AUTHORITY_PATTERN);
  if (hostMatch === null) {
    return null;
  }
