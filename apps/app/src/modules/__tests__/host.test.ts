...
describe('createModuleHost().launchSession — extraArgs sanitization', () => {
  it('strips a denied flag for a BUILT-IN module (not in extensionGrants)', async () => {
    // 'docs' is a compiled-in module: its id is absent from the grant map, yet it
    // takes remote input, so the denylist MUST still run. This is the regression
    // the fix guards: previously sanitize was gated on extensionGrants.has(moduleId).
    const host = createModuleHost('docs');
    ...
    expect(h.enterProjectFocus).toHaveBeenCalledWith(PROJECT.id);
...
describe('createModuleHost().relaunchSelf', () => {
  ...
  it('maps an error Result (ok:false — e.g. built-in/unknown) to false', async () => {
    ...
    expect(await createModuleHost('docs').relaunchSelf()).toBe(false);
  });
