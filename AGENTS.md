...
- **"Verify threads / CLI Agents still launch"** → `pnpm live:mode-reasoning`
  ...
  `live:mode-reasoning` launches Codex, Cursor, Codex, and OpenCode
  ...
  `live:memory` then attaches and proves a tagged hidden Codex thread
...
- **"Verify Memory plugin live retrieval" → `pnpm live:memory`.** Any change
  ...
  `POST /api/v1/plugins/memory/cli`, then spawns a tagged hidden Codex
...
- **The host owns extension-panel placement, not the extension
  ...
  generated starter `AGENTS.md`, and the `extension-creator` /
...
- **Plugin-contributed agent capabilities — skills + MCP servers.** New plugins
  ...
  `Codex` CLI processes outside ZCC's control — so they are declared in the
  ...
  `~/.Codex/skills/ext-<id>-<slug>/SKILL.md` (reusing the bundled-skill
...
- **Local (in-app authored) plugins are ordinary PluginService installs + a
  ...
  **including a `AGENTS.md`** the Creator auto-loads as project instructions
  ...
  baseProfile `Codex`, `permissionMode: 'acceptEdits'`) with its cwd bounded
...
- **Bundled skills + per-project MCP config are the app's runtime capability
  ...
  `~/.Codex/skills` at boot and via the "Reload skills & MCP" button
...
- **Launch IDENTITY is registry-dispatched (`apps/host-daemon/src/harness/registry.ts`), the
  ...
  *identity* — the concrete provider id, tab title, capability descriptor, and base
  ...
  `claudeCodeProvider` serves the whole Codex family, `shellProvider` serves
  ...
  fails if the `'Codex'` id reappears in `pty.ts`, and the existing
  `packages/domain/src/launch-provider.guard.test.ts` still forbids a re-rolled
  `isClaudeProfile` triplet — so keep the two `-suffix` Codex profile literals on
