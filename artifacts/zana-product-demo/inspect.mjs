...
for (const [time,cue] of cues) {
  await page.evaluate(t=>demo.seek(DemoTimeline.toPresentation(t)),time-.01);
...
if(process.argv.includes('--calibrate')) {
  const moves=await page.evaluate(()=>DemoTimeline.moves.map(m=>[DemoTimeline.toStory(m[0]),...m.slice(1)]));
  const corrected=moves.map(m=>[...m]);
  for(const point of points){
    const index=moves.findIndex(m=>m[0]===point.time);
    if(index<0)throw Error('Missing '+point.time);
    const old=moves[index];
    corrected[index]=[old[0],point.x,point.y,...old.slice(3)];
    if(index>0&&moves[index-1][1]===old[1]&&moves[index-1][2]===old[2])corrected[index-1]=[moves[index-1][0],point.x,point.y,...moves[index-1].slice(3)];
  }
  const file=path.join(root,'story-timeline.js');
...
const cliCues=[[51.8,'cli-new'],[52.8,'cli-mode'],[54,'draft'],[59,'cli-launch'],[62,'cli-fullscreen'],[70.7,'cli-close'],[73,'nav-gus']];
const cliPoints=[];
for(const [time,cue] of cliCues){
  await page.evaluate(t=>demo.seek(t-.01),time);
  const box=await page.locator(`[data-cue="${cue}"]`).first().boundingBox();
  if(!box)throw Error(`No CLI target ${cue}`);
  cliPoints.push({time,cue,x:Math.round(box.x+box.width/2),y:Math.round(box.y+box.height/2)});
}
if(process.argv.includes('--calibrate')){
  const file=path.join(root,'timeline.js');
  const moves=await page.evaluate(()=>DemoTimeline.cliMoves);
  const corrected=moves.map(m=>{const p=cliPoints.find(p=>p.time===m[0]);return p?[m[0],p.x,p.y,...m.slice(3)]:m;});
  const source=await readFile(file,'utf8');
  await writeFile(file,source.replace(/const cliMoves=\[[^;]*;/,'const cliMoves='+JSON.stringify(corrected)+';'));
  await page.reload();
}
await writeFile(path.join(root,'render/cli-cursor-targets.json'),JSON.stringify(cliPoints,null,2));
for(const t of [2,20,51.5,52,56,61,65.7,69,71.5,96,125,135]){
  await page.evaluate(t=>demo.seek(t),t);
  await page.screenshot({path:path.join(root,'render',`product-${t}.jpg`),type:'jpeg',quality:90});
}
for(const t of [2,15,31,42,51,55.5,66,71,75,90,98,103,108,124,132,147,151,156,163,168.5,175,184]){
  await page.evaluate(t=>demo.seek(DemoTimeline.toPresentation(t)),t);
