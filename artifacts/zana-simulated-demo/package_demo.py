...
VIDEO = ROOT / 'Zana-simulated-demo.mp4'
...
assert video['avg_frame_rate'] == '24/1' and int(video['nb_frames']) == 4560
assert audio['codec_name'] == 'aac' and audio['sample_rate'] == '48000'
assert abs(float(probe['format']['duration']) - 190) < .1
...
subprocess.run(['ffmpeg', '-v', 'error', '-y', '-ss', '73.8', '-i', str(VIDEO), '-frames:v', '1', '-q:v', '2', str(ROOT / 'thumbnail.jpg')], check=True)
report = {
    'ok': True,
...
'''
package = ROOT / 'Zana-simulated-demo-share.zip'
with zipfile.ZipFile(package, 'w', zipfile.ZIP_DEFLATED, compresslevel=6) as archive:
    ...
    for name in ['Zana-simulated-demo.mp4', 'thumbnail.jpg', 'captions.srt', 'captions.vtt', 'narration.md']:
    ...
    for name in ['index.html', 'style.css', 'timeline.js', 'app.js', 'captions.js', 'assets/zana.png', 'audio/narration.m4a']:
