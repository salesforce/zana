...
let browser;
try {
  execFileSync('unzip',['-q',path.join(root,'Zana-simulated-demo-share.zip'),'-d',temp]);
  ...
  assert.equal(await page.locator('video').evaluate(v=>v.duration),190);
  await page.locator('[data-time="119"]').click();
  await page.waitForFunction(()=>document.querySelector('video').currentTime>=119);
  ...
  const report={ok:true,offline:true,videoDuration:190,chapterSeek:119,interactivePlugin:true,pageErrors:errors,externalRequests:external.length};
