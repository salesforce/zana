...
assert video['avg_frame_rate'] == '24/1' and int(video['nb_frames']) == 3120
assert audio['codec_name'] == 'aac' and audio['sample_rate'] == '48000'
assert abs(float(probe['format']['duration']) - 130) < .1
...
subprocess.run(['ffmpeg', '-v', 'error', '-y', '-ss', '48', '-i', str(VIDEO), '-frames:v', '1', '-q:v', '2', str(ROOT / 'thumbnail.jpg')], check=True)
report = {
    'ok': True,
...
Play Zana-demo-fast.mp4, or open watch.html for chapter navigation.
...
'''
package = ROOT / 'Zana-demo-fast-share.zip'
with tempfile.TemporaryDirectory(prefix='.share-', dir=ROOT) as staging:
    ...
    with zipfile.ZipFile(staged_package, 'w', zipfile.ZIP_DEFLATED, compresslevel=6) as archive:
        ...
        for name in ['Zana-demo-fast.mp4', 'thumbnail.jpg', 'captions.srt', 'captions.vtt', 'narration.md', 'voice-preview.m4a', 'CREDITS.md']:
        ...
        for name in ['index.html', 'style.css', 'story-timeline.js', 'timeline.js', 'app.js', 'captions.js', 'assets/zana.png', 'audio/narration.m4a']:
