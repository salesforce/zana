...
export function createModuleHost(
  ...
  return {
    ...
    launchSession: async ({ projectId, personaId, extraArgs, title, cwd }) => {
      ...
      // is true of any module: a built-in like slack takes REMOTE input (an
      // inbound Slack `run <prompt>` → SlackBot → launchSession), so its launch
      // path is attacker-influenced just like a disk ext's. No module — built-in
      // or disk — has a legitimate reason to inject --dangerously-skip-permissions
