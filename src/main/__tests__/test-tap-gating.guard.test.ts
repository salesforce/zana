 ...
 * enforced by a handful of `if (E2E_TAP_ENABLED)` gates in `src/main/index.ts`
 * plus a `process.argv.includes('--zcc-e2e')` gate in `src/preload/index.ts`.
...
const INDEX_REL = 'main/index.ts';
const PRELOAD_REL = 'preload/index.ts';
...
describe('test-tap gating guard — the tap surface stays behind its flags', () => {
  const indexCode = stripComments(readFileSync(join(SRC_ROOT, INDEX_REL), 'utf8'));
  const indexLines = indexCode.split('\n');
  const preloadCode = stripComments(readFileSync(join(SRC_ROOT, PRELOAD_REL), 'utf8'));
  ...
  it('defines E2E_TAP_ENABLED exactly once from the ZCC_E2E env flag', () => {
    ...
    expect(defs.length, `expected ONE E2E_TAP_ENABLED definition in src/${INDEX_REL}`).toBe(1);
  ...
  it('records events at EXACTLY one site (the safeSend choke point)', () => {
    ...
    expect(
      n,
      `Expected exactly ONE \`testTap.record(\` call in src/${INDEX_REL} (only in ` +
  ...
  it('arms the tap (testTap.enable) ONLY inside an if (E2E_TAP_ENABLED) gate', () => {
    ...
    expect(
      ...
      `testTap.enable() at line ${at} in src/${INDEX_REL} is NOT gated by ` +
  ...
  it('registers every IPC.test.* handler ONLY inside an if (E2E_TAP_ENABLED) gate', () => {
    const handlerLines = linesContaining(indexLines, 'ipcMain.handle(IPC.test.');
    expect(
      handlerLines.length,
      `expected the 3 gated test IPC handlers (drainEvents/snapshot/reset) in src/${INDEX_REL}`
    ).toBeGreaterThanOrEqual(3);
    for (const ln of handlerLines) {
      expect(
        withinAny(gateRanges, ln),
        `ipcMain.handle(IPC.test.*) at line ${ln} in src/${INDEX_REL} is OUTSIDE ` +
          `every \`${GATE}\` block. The test IPC surface must be absent in prod.`
      ).toBe(true);
    }
  });
  ...
  it('exposes window.__zccTest in preload ONLY behind the --zcc-e2e argv gate', () => {
    ...
    expect(
      exposeLines.length,
      `expected exactly ONE window.__zccTest exposure in src/${PRELOAD_REL}`
    ).toBe(1);
    const gateLines = linesContaining(preloadLines, PRELOAD_GATE);
    expect(gateLines.length, `expected the --zcc-e2e argv gate in src/${PRELOAD_REL}`).toBe(1);
    const [start, end] = blockRange(preloadLines, gateLines[0]);
    const at = exposeLines[0];
    expect(
      at > start && at < end,
      `window.__zccTest exposure at line ${at} in src/${PRELOAD_REL} is OUTSIDE the ` +
