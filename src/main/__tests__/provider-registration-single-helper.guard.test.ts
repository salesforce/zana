 ...
 * `src/main/index.ts`. That single code path is invoked from the 3 lifecycle
...
describe('EPIC A / A.1 — providers are built ONLY inside rebuildProviders()', () => {
  const raw = readFileSync(join(SRC_ROOT, INDEX_REL), 'utf8');
  ...
  it('defines rebuildProviders exactly once', () => {
    expect(
      defLines.length,
      `Expected exactly ONE \`${FN_DEF}\` definition in src/${INDEX_REL}, found ` +
  ...
  it('constructs each LLM provider EXACTLY once', () => {
    for (const ctor of PROVIDER_CTORS) {
      ...
      expect(
        n,
        `Expected exactly ONE \`${ctor}\` in src/${INDEX_REL}, found ${n}. ` +
    ...
    for (const ctor of PROVIDER_CTORS) {
      ...
      expect(
        ...
        `\`${ctor}\` is at line ${ctorLine}, OUTSIDE the rebuildProviders body ` +
          `(${start}..${end}) in src/${INDEX_REL}. All LLM provider construction must ` +
  ...
  it('invokes rebuildProviders only from post-migration bootstrap and config updates', () => {
    expect(defLines.length).toBe(1);
    const defLine = defLines[0];
    const callLines = linesContaining(lines, FN_CALL).filter((ln) => ln !== defLine);
    expect(callLines.length, `Expected config:set + post-migration bootstrap calls, found line(s) ${callLines.join(', ') || '(none)'}`).toBe(2);
    expect(code).toMatch(/IPC\.config\.set,[\s\S]*rebuildProviders\(next\)/);
