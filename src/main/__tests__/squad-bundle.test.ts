...
describe('squad bundle workflow metadata', () => {
  ...
  it('rejects unsupported bundle versions and malformed workflow schema', () => {
    ...
    expect(sanitizeWorkflowMetadata({ ...workflow, modelTier: 'HIGH' })).toBeUndefined();
    expect(sanitizeWorkflowMetadata({ ...workflow, controller: { ...workflow.controller, modelTier: 'HIGH' } })).toBeUndefined();
    expect(sanitizeWorkflowMetadata({ ...workflow, workers: [{ ...workflow.workers[0], modelTier: 'HIGH' }] })).toBeUndefined();
  });
