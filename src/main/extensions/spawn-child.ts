...
// The env allowlist lives in an electron-free module (child-env.ts) so the
// brokered `ctx.exec` (broker-caps.ts, unit-tested, can't import `electron`)
// shares the SAME allowlist — the two untrusted-child spawn sites must trim env
// identically. Re-exported here for the existing spawn-child-env.test.
