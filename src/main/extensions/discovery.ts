...
import { isWithin, resolveContained, resolveContainedReal } from './path-util.js';
 ...
 * documented constant.
 *
 * Today `MAIN_MODULES` is `[slackMainModule]` — `slack` is the ONLY compiled-in
 * built-in (it was promoted from a disk extension; it needs in-process timers
 * for its live-bot poll loop). Zana is deliberately NOT reserved: it ships as a
 * disk extension (`extensions/zana/` id `zana`) and reaches its data through the
 * host MCP pool over the brokered `mcp` capability. Reserving it would reject its
 * own legitimate extension (the
 * id would "collide"), so each canonical extension protects its own namespace
 * simply by owning its directory — the id===dir-name rule then blocks any other
 * folder from claiming that id.
 *
 * This constant is only a FALLBACK: the authoritative caller (index.ts) forwards
 ...
 */
export const RESERVED_BUILTIN_IDS: readonly string[] = ['slack'];
