...
export class MigrationCasError extends Error {
  constructor() {
    super('Migration write rejected: file changed outside serialized transaction');
    this.name = 'MigrationCasError';
  }
}
...
export function durableRemove(
  ...
  const fs = options.fs ?? nodeFileSystem;
  if (currentHash(fs, target) !== options.expectedHash) throw new MigrationCasError();
  fs.unlink(target);
  fsyncDirectory(fs, dirname(target));
}
...
const processTransactionQueue = createSerializedTransactionQueue();

export function runSerializedMigrationTransaction<T>(task: () => Promise<T>): Promise<T> {
  return processTransactionQueue.run(task);
}
