...
import type { HarnessTranscriptAdapter, NativeSessionPatch, TranscriptSessionRef } from './harness/session-adapter.js';

export type { TranscriptSessionRef } from './harness/session-adapter.js';
...
export class TranscriptSource {
  ...
  private async resolve(ref: TranscriptSessionRef): Promise<{
    ...
    const registration = registrationFor(ref.profile as import('../shared/types.js').LaunchProfileId);
