...
let hostReact = null;
const GLOBAL_KEY$1 = "__ZCC_HOST_REACT__";
...
function getHostReact() {
  ...
  throw new Error(
    "zana extension: host React unavailable — the host must set globalThis." + GLOBAL_KEY$1 + " before import, or call activate({ React })."
  );
}
const Fragment = Symbol.for("zana.jsx.Fragment");
...
function useStore(api, selector = identity) {
  const slice = reactDefault.useSyncExternalStore(
    api.subscribe,
    reactDefault.useCallback(() => selector(api.getState()), [api, selector]),
    reactDefault.useCallback(() => selector(api.getInitialState()), [api, selector])
  );
...
function srcArgs(s) {
  return s.kind === "global" ? { useGlobal: true } : { projectPath: s.projectPath, useGlobal: false };
}
function getSnapshot(src) {
  return callZana("getSnapshot", srcArgs(src));
}
function getTicket(src, id) {
  return callZana("getTicket", { ...srcArgs(src), id });
}
function getArtifact(src, id) {
  return callZana("getArtifact", { ...srcArgs(src), id });
}
...
function assignTicket(src, id, patch2) {
  return callZana("assignTicket", { ...srcArgs(src), id, ...patch2 });
}
...
const SUB_TABS = ["tickets", "sprints", "docs", "profiles"];
const DEFAULT_SUB_TAB = "tickets";
...
function writeActiveTab(tab2) {
  safeSet(ACTIVE_TAB_KEY, tab2);
}
...
function writeCollapsed(projectId, map2) {
  safeSet(collapsedKey(projectId), JSON.stringify(map2));
}
...
function sourceOf(key) {
  return key.useGlobal ? { kind: "global" } : { kind: "project", projectPath: key.projectPath ?? "" };
}
function emptyEntry() {
  return {
    snapshot: null,
...
const assignTimers = /* @__PURE__ */ new Map();
const assignPrev = /* @__PURE__ */ new Map();
const dismissTimers = /* @__PURE__ */ new Map();
const refreshIntervals = /* @__PURE__ */ new Map();
const refreshRefcounts = /* @__PURE__ */ new Map();
let globalProfiles = [];
let globalProfilesLoadedAt = null;
let globalProfilesInflight = null;
function timerKey(mapKey, ticketId) {
  return `${mapKey}::${ticketId}`;
}
function mirrorProfilesToEntry(set, mapKey) {
  set((s) => {
    const entry2 = s.byKey[mapKey] ?? emptyEntry();
    if (entry2.profiles === globalProfiles && entry2.profilesLoadedAt === globalProfilesLoadedAt) {
      return {};
    }
    return {
      byKey: {
        ...s.byKey,
        [mapKey]: { ...entry2, profiles: globalProfiles, profilesLoadedAt: globalProfilesLoadedAt }
      }
    };
  });
}
function patchAssignee(set, mapKey, ticketId, patch2) {
  set((s) => {
    const entry2 = s.byKey[mapKey];
    if (!(entry2 == null ? void 0 : entry2.snapshot)) return {};
    const snap = entry2.snapshot;
    return {
      byKey: {
        ...s.byKey,
        [mapKey]: {
          ...
          snapshot: {
            ...snap,
            tickets: snap.tickets.map((t) => t.id === ticketId ? { ...t, ...patch2 } : t)
          }
        }
      }
    };
...
const useTickets = create$1((set, get) => ({
  byKey: {},
  assignUndo: {},
  ...
  openSprint: (sprintId) => {
    ...
    const entry2 = get().byKey[k];
    if ((entry2 == null ? void 0 : entry2.loadedAt) != null) {
      void get().loadProfiles(key);
      return;
    }
    if (entry2 == null ? void 0 : entry2.loading) return;
  ...
  hydrateAll: async (projects) => {
    const list2 = projects ?? [];
    for (const p of list2) {
      const k = mapKeyOf({ projectId: p.id });
      const entry2 = get().byKey[k];
      if ((entry2 == null ? void 0 : entry2.loadedAt) != null || (entry2 == null ? void 0 : entry2.loading)) continue;
      await get().refresh(
  ...
  refresh: async (key, opts) => {
    const background = (opts == null ? void 0 : opts.background) ?? false;
    const k = mapKeyOf(key);
    set((s) => {
      const entry2 = s.byKey[k] ?? emptyEntry();
      return {
        byKey: {
          ...s.byKey,
          [k]: {
    ...
    try {
      ...
      set((s) => {
        const entry2 = s.byKey[k] ?? emptyEntry();
        if (background) {
          return { byKey: { ...s.byKey, [k]: { ...entry2, refreshing: false } } };
        }
        return {
          byKey: {
            ...s.byKey,
    ...
    if (!globalProfilesInflight) {
      globalProfilesInflight = (async () => {
        ...
        try {
          ...
          if (Array.isArray(fetched)) list2 = fetched;
        } catch {
        }
        globalProfiles = list2;
  ...
  applyAssign: (key, ticket, choice) => {
    ...
    if (choice.kind === "clear") {
      patch2 = { assigneeName: void 0, assigneeId: void 0, assigneeProfileId: void 0 };
      args = { profileId: null };
      label = "Unassigned";
    } else if (choice.kind === "profile") {
      patch2 = { assigneeName: choice.displayName, assigneeProfileId: choice.profileId };
      args = { profileId: choice.profileId };
      label = choice.displayName;
    } else {
      patch2 = { assigneeName: choice.assigneeName, assigneeProfileId: void 0 };
      args = { assigneeName: choice.assigneeName };
      label = choice.assigneeName;
    }
    patchAssignee(set, k, ticket.id, patch2);
  ...
  startAutoRefresh: (key) => {
    ...
    if (next === 1 && !refreshIntervals.has(k)) {
      const handle2 = setInterval(() => {
        var _a;
        const s = get();
        if (s.autoRefresh && !((_a = s.byKey[k]) == null ? void 0 : _a.paused)) void s.refresh(key, { background: true });
      }, AUTO_REFRESH_MS);
      refreshIntervals.set(k, handle2);
    }
  },
  stopAutoRefresh: (key) => {
    const k = mapKeyOf(key);
    ...
    if (cur <= 1) {
      ...
      const handle2 = refreshIntervals.get(k);
      if (handle2) {
  ...
  setPaused: (key, paused) => {
    ...
    set((s) => {
      const entry2 = s.byKey[k];
      if (!entry2 || entry2.paused === paused) return {};
      return { byKey: { ...s.byKey, [k]: { ...entry2, paused } } };
    });
  },
  toggleColumnCollapsed: (key, status, defaultCollapsed) => {
    ...
    set((s) => {
      const entry2 = s.byKey[k] ?? emptyEntry();
      const next = {
        ...entry2.collapsedColumns,
        [statusKey]: !(entry2.collapsedColumns[statusKey] ?? defaultCollapsed)
      };
      writeCollapsed(k, next);
      return { byKey: { ...s.byKey, [k]: { ...entry2, collapsedColumns: next } } };
    });
  },
...
function useTicketsKpis(key) {
  return useTickets((s) => {
    var _a, _b;
    return ((_b = (_a = s.byKey[mapKeyOf(key)]) == null ? void 0 : _a.snapshot) == null ? void 0 : _b.kpis) ?? null;
  });
}
const STATUS_ORDER = [
  "backlog",
...
function resolveSprintName(sprintId, sprints) {
  ...
  const name2 = (_a = sprints.find((s) => s.id === sprintId)) == null ? void 0 : _a.name;
  if (!name2) return void 0;
  if (name2 === `Sprint ${sprintId.slice(0, 8)}`) return void 0;
  return name2;
}
...
function buildColumns(tickets) {
  ...
  for (const t of tickets) {
    ...
    (map2.get(key) ?? map2.set(key, []).get(key)).push(t);
  }
  for (const items of map2.values()) {
    items.sort((a, b) => substanceScore(b) - substanceScore(a));
  }
  return [...map2.entries()].sort(([a], [b]) => {
...
function buildProfileMap(profiles) {
  const map2 = /* @__PURE__ */ new Map();
  for (const p of profiles) if (p && typeof p.id === "string") map2.set(p.id, p);
  return map2;
}
...
function initials$1(name2) {
  return name2.split(/\s+/).slice(0, 2).map((p) => {
    var _a;
...
function AssignMenu({
  ...
  return /* @__PURE__ */ jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsx("div", { className: "zana-assign-menu-label", children: "Assign to" }),
        /* @__PURE__ */ jsxs(
          "button",
          {
            type: "button",
            ...
            children: [
              /* @__PURE__ */ jsx("span", { className: "zana-assign-item-icon", "aria-hidden": true, children: /* @__PURE__ */ jsx(UserX, { size: 13 }) }),
              /* @__PURE__ */ jsx("span", { className: "zana-assign-item-name", children: "Unassigned" })
            ]
          }
        ),
        /* @__PURE__ */ jsxs("div", { className: "zana-assign-menu-list", children: [
          profiles.length === 0 && /* @__PURE__ */ jsx("div", { className: "zana-assign-menu-empty", children: "No workspace profiles." }),
          profiles.map((p) => /* @__PURE__ */ jsxs(
            "button",
            {
              type: "button",
              role: "menuitem",
              ...
              children: [
                /* @__PURE__ */ jsx("span", { className: "zana-assign-item-icon", "aria-hidden": true, children: p.icon ?? "🤖" }),
                /* @__PURE__ */ jsx("span", { className: "zana-assign-item-name", children: p.displayName }),
                p.category && /* @__PURE__ */ jsx("span", { className: "zana-assign-item-cat", children: p.category })
              ]
            },
            p.id
          ))
        ...
        /* @__PURE__ */ jsxs(
          "form",
          {
            ...
            children: [
              ...
              /* @__PURE__ */ jsx("button", { type: "submit", disabled: !name2.trim(), title: "Assign to this name", children: /* @__PURE__ */ jsx(UserPlus, { size: 13 }) })
            ]
          }
        )
      ]
...
function unwrapBareFence(text2) {
  ...
  if (openLine === -1 || closeLine === -1 || openLine === closeLine) return text2;
  for (let i = closeLine + 1; i < lines.length; i++) {
    if (lines[i].trim().length > 0) return text2;
  }
  const head = lines.slice(0, openLine).join("\n").trimEnd();
  const headHasBlockMarkdown = /(^|\n)\s*(#{1,6} |[-*] |\d+\. |\| |-{3,}\s*$|={3,}\s*$)/.test(
    head
  );
  if (headHasBlockMarkdown) return text2;
  const inner = lines.slice(openLine + 1, closeLine).filter((l) => !fenceRe.test(l)).join("\n");
  const looksLikeMarkdown = /(^|\n)(#{1,6} |[-*] |\| .* \||\*\*[^*]+\*\*)/.test(inner);
  if (!looksLikeMarkdown) return text2;
  return head ? `${head}

${inner}` : inner;
}
function MarkdownContent({ text: text2 }) {
  const body = unwrapBareFence(text2);
  return /* @__PURE__ */ jsx("div", { className: "inbox-md zana-md", children: /* @__PURE__ */ jsx(
    Markdown,
    {
      ...
      components: {
        ...
        // react-markdown v10).
        a: (props) => /* @__PURE__ */ jsx("a", { ...props, target: "_blank", rel: "noreferrer" }),
        // GFM tables get a wrapper so horizontal overflow scrolls within the
        // modal instead of stretching it.
        table: (props) => /* @__PURE__ */ jsx("div", { className: "zana-md-table-wrap", children: /* @__PURE__ */ jsx("table", { ...props }) })
      },
      children: body
    }
  ) });
...
function TicketDetailModal({
  ...
  return /* @__PURE__ */ jsx("div", { className: "palette-backdrop", onMouseDown: onClose, children: /* @__PURE__ */ jsx(
    "div",
    {
      ...
      children: selection.kind === "ticket" ? /* @__PURE__ */ jsx(
        TicketDetail,
        {
          host,
      ...
      ) : selection.kind === "artifact" ? /* @__PURE__ */ jsx(
        ArtifactDetail,
        {
      ...
      ) : /* @__PURE__ */ jsx(ProfileDetail, { profile: selection.profile, tickets, onClose })
    }
  ) });
}
...
function AuditDetail({ entry: entry2 }) {
  const d = entry2.details ?? {};
  const action = entry2.action;
  if (action === "status_changed" && ("from" in d || "to" in d)) {
    return /* @__PURE__ */ jsxs("div", { className: "zana-timeline-detail", children: [
      d.from != null && /* @__PURE__ */ jsx("span", { className: "zana-status-from", children: detailValue(d.from) }),
      /* @__PURE__ */ jsx(ArrowRight, { size: 11, className: "zana-status-arrow", "aria-hidden": true }),
      /* @__PURE__ */ jsx("span", { className: "zana-status-to", children: detailValue(d.to) })
    ] });
  }
  if (action === "claimed" && "agentName" in d) {
    return /* @__PURE__ */ jsxs("div", { className: "zana-timeline-detail", children: [
      "claimed by ",
      /* @__PURE__ */ jsx("strong", { children: detailValue(d.agentName) })
    ] });
  }
  if (action === "assigned") {
    ...
    return /* @__PURE__ */ jsxs("div", { className: "zana-timeline-detail", children: [
      "assigned to ",
      /* @__PURE__ */ jsx("strong", { children: who || "someone" }),
      d.from != null && detailValue(d.from) && /* @__PURE__ */ jsxs("span", { className: "zana-detail-pill", children: [
        /* @__PURE__ */ jsx("span", { className: "zana-detail-key", children: "from" }),
        detailValue(d.from)
      ] })
    ] });
  }
  if (action === "unassigned") {
    return /* @__PURE__ */ jsxs("div", { className: "zana-timeline-detail", children: [
      "unassigned",
      d.from != null && detailValue(d.from) && /* @__PURE__ */ jsxs("span", { className: "zana-detail-pill", children: [
        /* @__PURE__ */ jsx("span", { className: "zana-detail-key", children: "was" }),
        detailValue(d.from)
      ] })
    ] });
  }
  if (action === "created") {
    return /* @__PURE__ */ jsxs("div", { className: "zana-timeline-detail", children: [
      "created",
      d.priority != null && /* @__PURE__ */ jsx("span", { className: `zana-prio zana-prio--${detailValue(d.priority).toLowerCase()}`, children: detailValue(d.priority) })
    ] });
  }
  const keys2 = Object.keys(d);
  if (keys2.length === 0) return null;
  return /* @__PURE__ */ jsx("div", { className: "zana-timeline-detail", children: keys2.map((k) => /* @__PURE__ */ jsxs("span", { className: "zana-detail-pill", children: [
    /* @__PURE__ */ jsx("span", { className: "zana-detail-key", children: k }),
    detailValue(d[k])
  ] }, k)) });
}
...
function TicketDetail({
  ...
  const [detail, setDetail] = useState({ ...ticket, audit: [] });
  ...
  const sprint = detail.sprintId ? sprints.find((s) => s.id === detail.sprintId) : void 0;
  const sprintLabel = (sprint == null ? void 0 : sprint.name) ?? (detail.sprintId ? shortId(detail.sprintId) : void 0);
  ...
  const handleAssign = (choice) => {
    setAssignOpen(false);
    if (choice.kind === "clear") {
      setLocalAssignee({ assigneeName: void 0, assigneeProfileId: void 0 });
    } else if (choice.kind === "profile") {
      setLocalAssignee({ assigneeName: choice.displayName, assigneeProfileId: choice.profileId });
    } else {
      setLocalAssignee({ assigneeName: choice.assigneeName, assigneeProfileId: void 0 });
    }
    onAssign(choice);
  };
  const targetProject = (() => {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      if (!full) {
        toast(
          `Couldn't load the “${(prof == null ? void 0 : prof.displayName) ?? assigneeProfileId}” profile — not started`,
          "error"
        );
        return;
      }
  ...
  const ticketTitleById = (id) => {
    var _a;
    return (_a = tickets.find((t) => t.id === id)) == null ? void 0 : _a.title;
  };
  return /* @__PURE__ */ jsxs(Fragment, { children: [
    /* @__PURE__ */ jsxs("header", { className: "gus-modal-header", children: [
      /* @__PURE__ */ jsxs("div", { className: "gus-modal-title", children: [
        /* @__PURE__ */ jsxs("span", { className: "gus-card-type", children: [
          /* @__PURE__ */ jsx(Ticket, { size: 14, "aria-hidden": true }),
          /* @__PURE__ */ jsx("span", { children: shortId(detail.id) })
        ] }),
        detail.priority && /* @__PURE__ */ jsx("span", { className: `zana-prio zana-prio--${detail.priority.toLowerCase()}`, children: detail.priority }),
        detail.blockedBy.length > 0 && /* @__PURE__ */ jsxs("span", { className: "zana-blocked-tag", title: `Blocked by ${detail.blockedBy.length}`, children: [
          /* @__PURE__ */ jsx(Ban, { size: 11, "aria-hidden": true }),
          " Blocked"
        ] })
      ] }),
      /* @__PURE__ */ jsx("div", { className: "gus-modal-header-actions", children: /* @__PURE__ */ jsx("button", { type: "button", className: "icon-btn", "aria-label": "Close", onClick: onClose, children: /* @__PURE__ */ jsx(X, { size: 14 }) }) })
    ] }),
    /* @__PURE__ */ jsxs("div", { className: "gus-modal-body", children: [
      /* @__PURE__ */ jsx("h3", { className: "gus-modal-subject", children: detail.title }),
      ...
      /* @__PURE__ */ jsxs("div", { className: "zana-action-bar", children: [
        /* @__PURE__ */ jsx("div", { className: "zana-action-assignee", children: assigneeName ? /* @__PURE__ */ jsxs(Fragment, { children: [
          /* @__PURE__ */ jsx("span", { className: "zana-action-avatar", "aria-hidden": true, children: prof ? prof.icon : initials(assigneeName) }),
          /* @__PURE__ */ jsxs("span", { className: "zana-action-assignee-text", children: [
            /* @__PURE__ */ jsx("span", { className: "zana-action-assignee-name", children: assigneeName }),
            prof && /* @__PURE__ */ jsx("span", { className: "zana-action-assignee-sub", children: prof.displayName })
          ] })
        ] }) : /* @__PURE__ */ jsxs(Fragment, { children: [
          /* @__PURE__ */ jsx("span", { className: "zana-action-avatar zana-action-avatar--empty", "aria-hidden": true, children: /* @__PURE__ */ jsx(User, { size: 13 }) }),
          /* @__PURE__ */ jsx("span", { className: "zana-card-unassigned", children: "Unassigned" })
        ] }) }),
        /* @__PURE__ */ jsxs("div", { className: "zana-action-controls", children: [
          assigneeProfileId && /* @__PURE__ */ jsxs(
            "button",
            {
              type: "button",
              ...
              title: startTitle,
              children: [
                launching ? /* @__PURE__ */ jsx(LoaderCircle, { size: 14, className: "gus-spin", "aria-hidden": true }) : /* @__PURE__ */ jsx(Play, { size: 14, "aria-hidden": true }),
                launching ? "Starting…" : "Start"
              ]
            }
          ),
          /* @__PURE__ */ jsxs("span", { className: "zana-assign-row-control", children: [
            /* @__PURE__ */ jsxs(
              "button",
              {
                type: "button",
                ...
                children: [
                  ...
                  /* @__PURE__ */ jsx(ChevronDown, { size: 12, "aria-hidden": true })
                ]
              }
            ),
            assignOpen && /* @__PURE__ */ jsx(
              AssignMenu,
              {
                profiles,
                onPick: handleAssign,
      ...
      detail.labels.length > 0 && /* @__PURE__ */ jsx("div", { className: "zana-modal-labels", children: detail.labels.map((l) => /* @__PURE__ */ jsxs("span", { className: "zana-label-chip", children: [
        /* @__PURE__ */ jsx(Tag, { size: 10, "aria-hidden": true }),
        " ",
        l
      ] }, l)) }),
      detail.blockedBy.length > 0 && /* @__PURE__ */ jsxs("div", { className: "zana-modal-section", children: [
        /* @__PURE__ */ jsxs("div", { className: "gus-modal-section-label", children: [
          /* @__PURE__ */ jsx(Ban, { size: 12, "aria-hidden": true }),
          " Blocked by"
        ] }),
        /* @__PURE__ */ jsx("div", { className: "zana-modal-blocked-ids", children: detail.blockedBy.map((id) => {
          const title = ticketTitleById(id);
          return /* @__PURE__ */ jsxs("span", { className: "gus-chip zana-blocker-chip", title: id, children: [
            title ?? shortId(id),
            title && /* @__PURE__ */ jsx("span", { className: "zana-blocker-id", children: shortId(id) })
          ] }, id);
        }) })
      ] }),
      /* @__PURE__ */ jsxs("div", { className: "zana-modal-section", children: [
        /* @__PURE__ */ jsx("div", { className: "gus-modal-section-label", children: "Description" }),
        detail.description ? /* @__PURE__ */ jsx(MarkdownContent, { text: detail.description }) : /* @__PURE__ */ jsx("div", { className: "gus-modal-empty", children: "No description." })
      ] }),
      detail.resultSummary && /* @__PURE__ */ jsxs("div", { className: "zana-modal-section", children: [
        /* @__PURE__ */ jsx("div", { className: "gus-modal-section-label", children: "Result summary" }),
        /* @__PURE__ */ jsx(MarkdownContent, { text: detail.resultSummary })
      ] }),
      /* @__PURE__ */ jsxs("div", { className: "zana-modal-section gus-modal-chatter", children: [
        /* @__PURE__ */ jsxs("div", { className: "gus-modal-section-label", children: [
          /* @__PURE__ */ jsx(MessageSquare, { size: 12, "aria-hidden": true }),
          " Comments",
          comments.length > 0 && /* @__PURE__ */ jsx("span", { className: "gus-chatter-count", children: comments.length })
        ] }),
        comments.length === 0 ? /* @__PURE__ */ jsx("div", { className: "gus-modal-empty", children: "No comments." }) : /* @__PURE__ */ jsx("ul", { className: "gus-chatter-list", children: comments.map((c, i) => /* @__PURE__ */ jsxs("li", { className: "gus-chatter-post", children: [
          /* @__PURE__ */ jsx("div", { className: "gus-chatter-avatar", "aria-hidden": true, children: initials(c.author ?? "?") }),
          /* @__PURE__ */ jsxs("div", { className: "gus-chatter-main", children: [
            /* @__PURE__ */ jsxs("div", { className: "gus-chatter-head", children: [
              /* @__PURE__ */ jsx("span", { className: "gus-chatter-author", children: c.author ?? "Unknown" }),
              /* @__PURE__ */ jsx("span", { className: "gus-chatter-time", children: fmtDateTime$1(c.createdAt) })
            ] }),
            /* @__PURE__ */ jsx("div", { className: "gus-chatter-body", children: /* @__PURE__ */ jsx(MarkdownContent, { text: c.body }) })
          ] })
        ] }, i)) })
      ] }),
      /* @__PURE__ */ jsxs("div", { className: "zana-modal-section", children: [
        /* @__PURE__ */ jsxs("div", { className: "gus-modal-section-label", children: [
          /* @__PURE__ */ jsx(History, { size: 12, "aria-hidden": true }),
          " Activity",
          audit.length > 0 && /* @__PURE__ */ jsx("span", { className: "gus-chatter-count", children: audit.length }),
          loading && /* @__PURE__ */ jsx(LoaderCircle, { size: 12, className: "gus-spin zana-timeline-spinner", "aria-label": "Loading activity" })
        ] }),
        audit.length === 0 ? /* @__PURE__ */ jsx("div", { className: "gus-modal-empty", children: loading ? "Loading activity…" : "No activity recorded." }) : /* @__PURE__ */ jsx("ol", { className: "zana-timeline", children: audit.map((entry2, i) => /* @__PURE__ */ jsxs("li", { className: "zana-timeline-item", children: [
          /* @__PURE__ */ jsx("span", { className: "zana-timeline-dot", "aria-hidden": true }),
          /* @__PURE__ */ jsxs("div", { className: "zana-timeline-main", children: [
            /* @__PURE__ */ jsxs("div", { className: "zana-timeline-head", children: [
              /* @__PURE__ */ jsx("span", { className: "zana-timeline-action", children: entry2.action || "event" }),
              entry2.actor && /* @__PURE__ */ jsx("span", { className: "zana-timeline-actor", children: entry2.actor }),
              entry2.timestamp && /* @__PURE__ */ jsx("span", { className: "zana-timeline-time", children: fmtDateTime$1(entry2.timestamp) })
            ] }),
            /* @__PURE__ */ jsx(AuditDetail, { entry: entry2 })
          ] })
        ] }, entry2.id ?? i)) })
      ] })
    ] })
...
function ArtifactDetail({
  ...
  const [content2, setContent] = useState(artifact.content);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    let live = true;
    setLoading(true);
  ...
  return /* @__PURE__ */ jsxs(Fragment, { children: [
    /* @__PURE__ */ jsxs("header", { className: "gus-modal-header", children: [
      /* @__PURE__ */ jsxs("div", { className: "gus-modal-title", children: [
        ...
        artifact.type && /* @__PURE__ */ jsx("span", { className: "zana-type-badge", children: artifact.type })
      ] }),
      /* @__PURE__ */ jsx("div", { className: "gus-modal-header-actions", children: /* @__PURE__ */ jsx("button", { type: "button", className: "icon-btn", "aria-label": "Close", onClick: onClose, children: /* @__PURE__ */ jsx(X, { size: 14 }) }) })
    ] }),
    /* @__PURE__ */ jsxs("div", { className: "gus-modal-body", children: [
      /* @__PURE__ */ jsx("h3", { className: "gus-modal-subject", children: artifact.title }),
      shownFacts.length > 0 && /* @__PURE__ */ jsx("dl", { className: "gus-facts", children: shownFacts.map(([k, v]) => /* @__PURE__ */ jsxs("div", { className: "gus-fact", children: [
        /* @__PURE__ */ jsx("dt", { children: k }),
        /* @__PURE__ */ jsx("dd", { children: v })
      ] }, k)) }),
      artifact.tags.length > 0 && /* @__PURE__ */ jsx("div", { className: "zana-modal-labels", children: artifact.tags.map((t) => /* @__PURE__ */ jsxs("span", { className: "zana-label-chip", children: [
        /* @__PURE__ */ jsx(Tag, { size: 10, "aria-hidden": true }),
        " ",
        t
      ] }, t)) }),
      /* @__PURE__ */ jsxs("div", { className: "zana-modal-section", children: [
        /* @__PURE__ */ jsx("div", { className: "gus-modal-section-label", children: "Content" }),
        loading && !content2 ? /* @__PURE__ */ jsxs("div", { className: "gus-modal-loading", children: [
          /* @__PURE__ */ jsx(LoaderCircle, { size: 14, className: "gus-spin" }),
          " Loading content…"
        ] }) : content2 ? /* @__PURE__ */ jsx("div", { className: "zana-doc-reader", children: /* @__PURE__ */ jsx(MarkdownContent, { text: content2 }) }) : /* @__PURE__ */ jsx("div", { className: "gus-modal-empty", children: "No content." })
      ] })
    ] })
  ] });
}
function ProfileDetail({
  ...
  return /* @__PURE__ */ jsxs(Fragment, { children: [
    /* @__PURE__ */ jsxs("header", { className: "gus-modal-header", children: [
      /* @__PURE__ */ jsxs("div", { className: "gus-modal-title", children: [
        /* @__PURE__ */ jsx("span", { className: "zana-profile-modal-icon", "aria-hidden": true, children: detail.icon ?? "🤖" }),
        /* @__PURE__ */ jsx(
          "span",
          {
        ...
        detail.model && /* @__PURE__ */ jsxs("span", { className: "zana-type-badge zana-profile-model", children: [
          /* @__PURE__ */ jsx(Cpu, { size: 11, "aria-hidden": true }),
          " ",
          detail.model
        ] })
      ] }),
    ...
    /* @__PURE__ */ jsxs("div", { className: "gus-modal-body", children: [
      /* @__PURE__ */ jsx("h3", { className: "gus-modal-subject", children: detail.displayName }),
      detail.description && /* @__PURE__ */ jsx("p", { className: "zana-profile-desc", children: detail.description }),
      shownFacts.length > 0 && /* @__PURE__ */ jsx("dl", { className: "gus-facts", children: shownFacts.map(([k, v]) => /* @__PURE__ */ jsxs("div", { className: "gus-fact", children: [
        /* @__PURE__ */ jsx("dt", { children: k }),
        /* @__PURE__ */ jsx("dd", { children: v })
      ] }, k)) }),
      (allowed.length > 0 || disallowed.length > 0) && /* @__PURE__ */ jsxs("div", { className: "zana-modal-section", children: [
        /* @__PURE__ */ jsx("div", { className: "gus-modal-section-label", children: "Tools" }),
        allowed.length > 0 && /* @__PURE__ */ jsxs("div", { className: "zana-profile-tools", children: [
          /* @__PURE__ */ jsxs("span", { className: "zana-profile-tools-label", children: [
            /* @__PURE__ */ jsx(CircleCheck, { size: 12, "aria-hidden": true }),
            " Allowed"
          ] }),
          /* @__PURE__ */ jsx("div", { className: "zana-profile-tool-chips", children: allowed.map((t) => /* @__PURE__ */ jsx("span", { className: "zana-label-chip zana-tool-chip zana-tool-chip--allow", children: t }, t)) })
        ] }),
        disallowed.length > 0 && /* @__PURE__ */ jsxs("div", { className: "zana-profile-tools", children: [
          /* @__PURE__ */ jsxs("span", { className: "zana-profile-tools-label", children: [
            /* @__PURE__ */ jsx(CircleX, { size: 12, "aria-hidden": true }),
            " Denied"
          ] }),
          /* @__PURE__ */ jsx("div", { className: "zana-profile-tool-chips", children: disallowed.map((t) => /* @__PURE__ */ jsx("span", { className: "zana-label-chip zana-tool-chip zana-tool-chip--deny", children: t }, t)) })
        ] })
      ] }),
      /* @__PURE__ */ jsxs("div", { className: "zana-modal-section", children: [
        /* @__PURE__ */ jsxs("div", { className: "gus-modal-section-label", children: [
        ...
        detail.systemPrompt ? /* @__PURE__ */ jsx("pre", { className: "zana-profile-prompt", children: detail.systemPrompt }) : /* @__PURE__ */ jsx("div", { className: "gus-modal-empty", children: loading ? "Loading system prompt…" : "No system prompt." })
      ] })
    ] })
  ] });
}
...
function ProfilesView({
  ...
  const groups = useMemo(() => {
    ...
    for (const p of profiles) {
      ...
      (map2.get(key) ?? map2.set(key, []).get(key)).push(p);
    }
    return [...map2.entries()];
  }, [profiles]);
  if (profiles.length === 0) {
    return /* @__PURE__ */ jsx("div", { className: "zana-profiles-view", children: /* @__PURE__ */ jsxs("div", { className: "gus-column-empty", children: [
      "No profiles found. Profiles live in ",
      /* @__PURE__ */ jsx("code", { children: "~/.zana/profiles/" }),
      " plus Zana's built-ins."
    ] }) });
  }
  return /* @__PURE__ */ jsxs("div", { className: "zana-profiles-view", children: [
    /* @__PURE__ */ jsxs("div", { className: "zana-profiles-summary", children: [
      ...
      workspaceCount > 0 && /* @__PURE__ */ jsxs(Fragment, { children: [
        " · ",
        /* @__PURE__ */ jsxs("span", { className: "zana-profiles-summary-ws", children: [
          workspaceCount,
          " workspace"
        ] })
      ] })
    ] }),
    groups.map(([category, items]) => /* @__PURE__ */ jsxs("section", { className: "zana-profile-group", children: [
      groups.length > 1 && /* @__PURE__ */ jsxs("div", { className: "zana-profile-group-head", children: [
        /* @__PURE__ */ jsx("span", { className: "zana-profile-group-title", children: category }),
        /* @__PURE__ */ jsx("span", { className: "zana-profile-group-count", children: items.length })
      ] }),
      /* @__PURE__ */ jsx("div", { className: "zana-profile-grid", children: items.map((p) => /* @__PURE__ */ jsx(
        ProfileCard,
        {
          profile: p,
...
function ProfileCard({
  ...
  return /* @__PURE__ */ jsxs(
    "article",
    {
      ...
      children: [
        /* @__PURE__ */ jsxs("div", { className: "zana-profile-card-top", children: [
          /* @__PURE__ */ jsx("span", { className: "zana-profile-card-icon", "aria-hidden": true, children: profile.icon ?? "🤖" }),
          /* @__PURE__ */ jsxs("div", { className: "zana-profile-card-id", children: [
            /* @__PURE__ */ jsx("h3", { className: "zana-profile-card-name", children: profile.displayName }),
            profile.category && /* @__PURE__ */ jsx("span", { className: "zana-profile-cat", children: profile.category })
          ] }),
          /* @__PURE__ */ jsx(
            "span",
            {
        ...
        profile.description && /* @__PURE__ */ jsx("p", { className: "zana-profile-card-desc", children: profile.description }),
        /* @__PURE__ */ jsxs("div", { className: "zana-profile-card-meta", children: [
          profile.model && /* @__PURE__ */ jsx("span", { className: "zana-label-chip", children: profile.model }),
          assignedCount > 0 && /* @__PURE__ */ jsxs("span", { className: "gus-chip", title: "Tickets assigned to this profile", children: [
            assignedCount,
            " assigned"
          ] })
        ] })
...
function excerptFromMarkdown(md, max = 180) {
  ...
  text2 = text2.replace(/^[ \t]*#{1,6}[ \t]+/gm, "");
  text2 = text2.replace(/^[ \t]*>[ \t]?/gm, "");
  text2 = text2.replace(/^[ \t]*[-*+][ \t]+/gm, "");
  text2 = text2.replace(/^[ \t]*\d+\.[ \t]+/gm, "");
...
function SprintsList({
  ...
  return /* @__PURE__ */ jsxs("div", { className: "zana-list", children: [
    sprints.length === 0 && /* @__PURE__ */ jsx("div", { className: "gus-column-empty", children: "No sprints." }),
    sprints.map((s) => /* @__PURE__ */ jsxs(
      "button",
      {
        type: "button",
        ...
        children: [
          /* @__PURE__ */ jsxs("div", { className: "zana-sprint-main", children: [
            /* @__PURE__ */ jsx("span", { className: "zana-sprint-name", children: s.name ?? shortId$1(s.id) }),
            s.status && /* @__PURE__ */ jsx("span", { className: "gus-chip", children: s.status })
          ] }),
          /* @__PURE__ */ jsxs("div", { className: "zana-sprint-counts", children: [
            /* @__PURE__ */ jsxs("span", { className: "zana-sprint-count", children: [
              /* @__PURE__ */ jsx(CircleDot, { size: 12, "aria-hidden": true }),
              " ",
              s.openCount ?? 0,
              " open"
            ] }),
...
function ArtifactCard({
  ...
  return /* @__PURE__ */ jsxs(
    "article",
    {
      ...
      children: [
        /* @__PURE__ */ jsx("div", { className: "zana-doc-icon", "aria-hidden": true, children: /* @__PURE__ */ jsx(BookOpen, { size: 18 }) }),
        /* @__PURE__ */ jsxs("div", { className: "zana-doc-body", children: [
          /* @__PURE__ */ jsxs("div", { className: "zana-doc-head", children: [
            /* @__PURE__ */ jsx("h3", { className: "zana-doc-title", children: artifact.title }),
            artifact.type && /* @__PURE__ */ jsx("span", { className: "zana-doc-type", children: artifact.type })
          ] }),
          excerpt && /* @__PURE__ */ jsx("p", { className: "zana-doc-excerpt", children: excerpt }),
          /* @__PURE__ */ jsxs("div", { className: "zana-doc-meta", children: [
            created && /* @__PURE__ */ jsxs("span", { className: "zana-doc-meta-item", children: [
              /* @__PURE__ */ jsx(CalendarRange, { size: 11, "aria-hidden": true }),
              " ",
              created
            ] }),
            artifact.createdBy && /* @__PURE__ */ jsxs("span", { className: "zana-doc-meta-item", children: [
              /* @__PURE__ */ jsx(User, { size: 11, "aria-hidden": true }),
              " ",
              artifact.createdBy
            ] }),
            linked > 0 && /* @__PURE__ */ jsxs("span", { className: "zana-doc-meta-item zana-doc-linked", title: "Linked tickets", children: [
              /* @__PURE__ */ jsx(Link2, { size: 11, "aria-hidden": true }),
              " ",
              linked,
              " linked ",
          ...
          artifact.tags.length > 0 && /* @__PURE__ */ jsxs("div", { className: "zana-doc-tags", children: [
            artifact.tags.slice(0, 5).map((t) => /* @__PURE__ */ jsxs("span", { className: "zana-label-chip", children: [
              /* @__PURE__ */ jsx(Tag, { size: 9, "aria-hidden": true }),
              " ",
              t
            ] }, t)),
            artifact.tags.length > 5 && /* @__PURE__ */ jsxs("span", { className: "gus-chip", children: [
              "+",
...
function DocsList({
  ...
  return /* @__PURE__ */ jsxs("div", { className: "zana-doc-list", children: [
    artifacts.length === 0 && /* @__PURE__ */ jsx("div", { className: "gus-column-empty", children: "No docs." }),
    artifacts.map((a) => /* @__PURE__ */ jsx(ArtifactCard, { artifact: a, onOpen }, a.id))
  ] });
}
function ProjectTicketsView({ host }) {
  const project = host.getActiveProject();
  if (!project) {
    return /* @__PURE__ */ jsx("section", { className: "gus-panel zana-panel", children: /* @__PURE__ */ jsx("div", { className: "empty-workspace overlay", children: /* @__PURE__ */ jsxs("div", { className: "empty-inner", children: [
      /* @__PURE__ */ jsx("h3", { children: "No project selected" }),
      /* @__PURE__ */ jsx("p", { children: "Open a project to see its Zana tickets." })
    ] }) }) });
  }
  return /* @__PURE__ */ jsx(TicketsBoard, { host, project });
}
function TicketsBoard({ host, project }) {
  ...
  const entry2 = useTicketsEntry(key);
  const kpis = useTicketsKpis(key);
  const snapshot = (entry2 == null ? void 0 : entry2.snapshot) ?? null;
  const loading = (entry2 == null ? void 0 : entry2.loading) ?? false;
  const error = (entry2 == null ? void 0 : entry2.error) ?? null;
  const [query, setQuery] = useState("");
  const [assigneeFilter, setAssigneeFilter] = useState(null);
  const collapsedOverrides = (entry2 == null ? void 0 : entry2.collapsedColumns) ?? {};
  const [selected, setSelected] = useState(null);
  const tickets = (snapshot == null ? void 0 : snapshot.tickets) ?? [];
  const sprints = (snapshot == null ? void 0 : snapshot.sprints) ?? [];
  const artifacts = (snapshot == null ? void 0 : snapshot.artifacts) ?? [];
  const profiles = (entry2 == null ? void 0 : entry2.profiles) ?? [];
  const profileMap = useMemo(() => buildProfileMap(profiles), [profiles]);
  const profileByName = useMemo(() => {
    const map2 = /* @__PURE__ */ new Map();
    for (const p of profiles) if (p && typeof p.displayName === "string") map2.set(p.displayName, p);
    return map2;
  }, [profiles]);
  ...
  const openTicket = (t) => setSelected({ kind: "ticket", ticket: t });
  const openProfile = (p) => setSelected({ kind: "profile", profile: p });
  const openArtifact = (a) => setSelected({ kind: "artifact", artifact: a });
  const isEmpty = !loading && !error && isSnapshotEmpty(snapshot);
  const isUninitialized = isEmpty && snapshot != null && !snapshot.isInitialized;
  const subTabBar = /* @__PURE__ */ jsxs("div", { className: "zana-tabs", role: "tablist", children: [
    ["tickets", "sprints", "docs", "profiles"].map((tab2) => /* @__PURE__ */ jsxs(
      "button",
      {
        type: "button",
        role: "tab",
        "aria-selected": activeSubTab === tab2,
        className: `zana-tab ${activeSubTab === tab2 ? "active" : ""}`,
        onClick: () => setActiveSubTab(tab2),
        children: [
          tab2 === "profiles" && /* @__PURE__ */ jsx(Users, { size: 13, "aria-hidden": true }),
          tab2 === "tickets" ? "Tickets" : tab2 === "sprints" ? "Sprints" : tab2 === "docs" ? "Docs" : "Profiles"
        ]
      },
      tab2
    )),
    sprintFilter && activeSubTab === "tickets" && /* @__PURE__ */ jsxs(
      "button",
      {
        type: "button",
        ...
        children: [
          "Sprint: ",
          ((_a = sprints.find((s) => s.id === sprintFilter)) == null ? void 0 : _a.name) ?? shortId$1(sprintFilter),
          /* @__PURE__ */ jsx(X, { size: 11 })
        ]
      }
    ),
    activeSubTab === "tickets" && /* @__PURE__ */ jsxs(
      "div",
      {
        ...
        children: [
          /* @__PURE__ */ jsx(
            "button",
            {
              ...
              children: /* @__PURE__ */ jsx(LayoutGrid, { size: 13, "aria-hidden": true })
            }
          ),
          /* @__PURE__ */ jsx(
            "button",
            {
              ...
              children: /* @__PURE__ */ jsx(List, { size: 13, "aria-hidden": true })
            }
          )
        ]
      }
    ),
    activeSubTab === "tickets" && /* @__PURE__ */ jsxs("div", { className: "zana-tabs-search gus-search", title: "Filter by title, labels & assignee", children: [
  ...
  if (error) {
    return /* @__PURE__ */ jsx("section", { className: "gus-panel zana-panel", children: /* @__PURE__ */ jsxs("div", { className: "gus-error", role: "alert", children: [
      /* @__PURE__ */ jsx(CircleAlert, { size: 16 }),
      /* @__PURE__ */ jsxs("div", { children: [
        /* @__PURE__ */ jsx("strong", { children: "Couldn't load Zana data." }),
        /* @__PURE__ */ jsx("p", { children: error })
      ] })
    ] }) });
  }
  if (loading && !snapshot) {
    return /* @__PURE__ */ jsx("section", { className: "gus-panel zana-panel", children: /* @__PURE__ */ jsx("div", { className: "gus-loading", children: "Loading Zana data…" }) });
  }
  if (isEmpty) {
    return /* @__PURE__ */ jsxs("section", { className: "gus-panel zana-panel", children: [
      subTabBar,
      activeSubTab === "profiles" ? /* @__PURE__ */ jsx(ProfilesView, { profiles, tickets, onOpen: openProfile }) : /* @__PURE__ */ jsx("div", { className: "empty-workspace overlay", children: /* @__PURE__ */ jsx("div", { className: "empty-inner", children: isUninitialized ? /* @__PURE__ */ jsxs(Fragment, { children: [
        ...
        initState.error && /* @__PURE__ */ jsx("p", { className: "gus-error", role: "alert", children: initState.error })
      ] }) : /* @__PURE__ */ jsxs(Fragment, { children: [
        /* @__PURE__ */ jsx("h3", { children: "No tickets yet" }),
        /* @__PURE__ */ jsxs("p", { children: [
          project.name,
          "'s ",
          /* @__PURE__ */ jsx("code", { children: ".zana/" }),
  ...
  return /* @__PURE__ */ jsxs("section", { className: "gus-panel zana-panel", children: [
    kpis && /* @__PURE__ */ jsxs("div", { className: "zana-kpi-strip", children: [
      /* @__PURE__ */ jsx(KpiCard, { icon: /* @__PURE__ */ jsx(CircleDot, { size: 15 }), label: "Open", value: kpis.openTickets, tone: "open" }),
      /* @__PURE__ */ jsx(KpiCard, { icon: /* @__PURE__ */ jsx(CircleCheck, { size: 15 }), label: "Closed", value: kpis.closedTickets, tone: "done" }),
      /* @__PURE__ */ jsx(KpiCard, { icon: /* @__PURE__ */ jsx(Ban, { size: 15 }), label: "Blocked", value: kpis.blockedTickets, tone: "blocked" }),
      /* @__PURE__ */ jsx(KpiCard, { icon: /* @__PURE__ */ jsx(Activity, { size: 15 }), label: "Throughput 7d", value: kpis.throughput7d ?? 0 }),
      /* @__PURE__ */ jsx(KpiCard, { icon: /* @__PURE__ */ jsx(CalendarRange, { size: 15 }), label: "Sprints", value: kpis.sprintCount }),
      /* @__PURE__ */ jsx(KpiCard, { icon: /* @__PURE__ */ jsx(FileText, { size: 15 }), label: "Docs", value: kpis.artifactCount }),
      /* @__PURE__ */ jsxs("div", { className: "zana-kpi-breakdowns", children: [
        /* @__PURE__ */ jsx(Breakdown, { title: "By status", counts: kpis.byStatus }),
        /* @__PURE__ */ jsx(Breakdown, { title: "By priority", counts: kpis.byPriority })
      ] })
    ] }),
    subTabBar,
    activeSubTab === "profiles" && /* @__PURE__ */ jsx(ProfilesView, { profiles, tickets, onOpen: openProfile }),
    activeSubTab === "sprints" && /* @__PURE__ */ jsx(SprintsList, { sprints, onOpenSprint: openSprint }),
    activeSubTab === "docs" && /* @__PURE__ */ jsx(DocsList, { artifacts, onOpen: openArtifact }),
    activeSubTab === "tickets" && assignees.length > 0 && /* @__PURE__ */ jsxs("div", { className: "zana-assignee-bar", role: "group", "aria-label": "Filter by assignee", children: [
      /* @__PURE__ */ jsxs(
        "button",
        {
          type: "button",
      ...
      assignees.map((name2) => {
        const prof = profileByName.get(name2);
        return /* @__PURE__ */ jsxs(
          "button",
          {
            type: "button",
            className: `zana-assignee-chip ${assigneeFilter === name2 ? "active" : ""}`,
            onClick: () => setAssigneeFilter(assigneeFilter === name2 ? null : name2),
            children: [
              /* @__PURE__ */ jsx(
                "span",
                {
                  className: "zana-assignee-chip-avatar",
                  style: { background: avatarColor(name2) },
                  "aria-hidden": true,
                  children: prof ? prof.icon : initials$1(name2)
                }
              ),
              name2
            ]
    ...
    activeSubTab === "tickets" && /* @__PURE__ */ jsx("div", { className: "zana-content", children: /* @__PURE__ */ jsx("div", { className: "gus-content", children: /* @__PURE__ */ jsxs("div", { className: "gus-board zana-board", children: [
      columns.length === 0 && /* @__PURE__ */ jsx("div", { className: "gus-column-empty", children: "No tickets match." }),
            ...
            children: [
              /* @__PURE__ */ jsxs(
                "button",
                {
                  ...
                  children: [
                    /* @__PURE__ */ jsxs("span", { className: "zana-column-head-left", children: [
                      collapsed ? /* @__PURE__ */ jsx(ChevronRight, { size: 13, "aria-hidden": true }) : /* @__PURE__ */ jsx(ChevronDown, { size: 13, "aria-hidden": true }),
                      /* @__PURE__ */ jsx("span", { className: "gus-column-title", children: status })
                    ] }),
                    /* @__PURE__ */ jsx("span", { className: "gus-column-count", children: items.length })
                  ]
                }
              ),
              !collapsed && (boardDensity === "list" ? /* @__PURE__ */ jsx("div", { className: "gus-column-body zana-column-body--dense", children: items.map((t) => /* @__PURE__ */ jsx(CompactTicketRow, { ticket: t, onOpen: () => openTicket(t) }, t.id)) }) : /* @__PURE__ */ jsx("div", { className: "gus-column-body", children: items.map((t) => /* @__PURE__ */ jsx(
                TicketCard,
                {
                  ticket: t,
...
function KpiCard({
  ...
  return /* @__PURE__ */ jsxs("div", { className: `zana-kpi-card ${tone ? `zana-kpi-card--${tone}` : ""}`, children: [
    /* @__PURE__ */ jsx("span", { className: "zana-kpi-icon", "aria-hidden": true, children: icon }),
    /* @__PURE__ */ jsx("span", { className: "zana-kpi-value", children: value }),
    /* @__PURE__ */ jsx("span", { className: "zana-kpi-label", children: label })
  ] });
}
function Breakdown({ title, counts }) {
  ...
  return /* @__PURE__ */ jsxs("div", { className: "zana-breakdown", children: [
    /* @__PURE__ */ jsx("div", { className: "zana-breakdown-title", children: title }),
    /* @__PURE__ */ jsx("div", { className: "zana-breakdown-rows", children: entries.map(([key, n]) => /* @__PURE__ */ jsxs("div", { className: "zana-breakdown-row", title: `${key}: ${n}`, children: [
      /* @__PURE__ */ jsx("span", { className: "zana-breakdown-key", children: key }),
      /* @__PURE__ */ jsx("span", { className: "zana-breakdown-bar", children: /* @__PURE__ */ jsx(
        "span",
        {
      ...
      /* @__PURE__ */ jsx("span", { className: "zana-breakdown-num", children: n })
    ] }, key)) })
  ] });
}
function TicketCard({
  ...
  const prio = (_a = ticket.priority) == null ? void 0 : _a.toLowerCase();
  const blurb = ((_b = ticket.description) == null ? void 0 : _b.trim()) || ((_c = ticket.resultSummary) == null ? void 0 : _c.trim());
  ...
  return /* @__PURE__ */ jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxs("div", { className: "gus-card-top zana-card-top", children: [
          /* @__PURE__ */ jsx(
            "span",
            {
          ...
          /* @__PURE__ */ jsx("div", { className: "gus-card-subject zana-card-title", children: ticket.title }),
          ticket.priority && /* @__PURE__ */ jsx("span", { className: `zana-prio zana-prio--${prio}`, children: ticket.priority })
        ] }),
        blurb && /* @__PURE__ */ jsx("div", { className: "zana-card-desc", children: blurb }),
        (ticket.type || ticket.blockedBy.length > 0 || sprintName || ticket.labels.length > 0) && /* @__PURE__ */ jsxs("div", { className: "zana-card-chips", children: [
          ticket.type && /* @__PURE__ */ jsx("span", { className: "zana-type-badge", children: ticket.type }),
          ticket.blockedBy.length > 0 && /* @__PURE__ */ jsxs("span", { className: "zana-blocked-tag", title: `Blocked by ${ticket.blockedBy.length}`, children: [
            /* @__PURE__ */ jsx(Ban, { size: 11, "aria-hidden": true }),
            " Blocked"
          ] }),
          sprintName && /* @__PURE__ */ jsx("span", { className: "gus-chip", children: sprintName }),
          ticket.labels.slice(0, 3).map((l) => /* @__PURE__ */ jsxs("span", { className: "zana-label-chip", children: [
            /* @__PURE__ */ jsx(Tag, { size: 9, "aria-hidden": true }),
            " ",
            l
          ] }, l)),
          extraLabels > 0 && /* @__PURE__ */ jsxs("span", { className: "gus-chip", children: [
            "+",
            extraLabels
          ] })
        ] }),
        /* @__PURE__ */ jsxs("div", { className: "zana-card-foot", children: [
          ticket.assigneeName ? /* @__PURE__ */ jsxs("span", { className: "zana-card-assignee", title: `Assigned to ${ticket.assigneeName}`, children: [
            /* @__PURE__ */ jsx(User, { size: 11, "aria-hidden": true }),
            " ",
            ticket.assigneeName
          ] }) : /* @__PURE__ */ jsx("span", { className: "zana-card-unassigned", children: "Unassigned" }),
          /* @__PURE__ */ jsx("span", { className: "zana-card-id", title: ticket.id, children: shortId$1(ticket.id) })
        ] })
      ]
    }
  );
  ...
  return /* @__PURE__ */ jsxs(
    "div",
    {
      ...
      children: [
        ...
        /* @__PURE__ */ jsx("span", { className: "zana-compact-title", children: ticket.title }),
        /* @__PURE__ */ jsx("span", { className: "zana-compact-id", title: ticket.id, children: shortId$1(ticket.id) })
      ]
    }
  );
}
...
function VersionSettings() {
  ...
  const copyUpgrade = useCallback(() => {
    if (!(info == null ? void 0 : info.upgradeCommand)) return;
  ...
  return /* @__PURE__ */ jsxs("div", { className: "settings-section zana-version-settings", children: [
    /* @__PURE__ */ jsxs("p", { className: "settings-help", children: [
      "The Zana MCP server (",
    ...
    /* @__PURE__ */ jsxs("div", { className: "claude-scope-card", children: [
      /* @__PURE__ */ jsxs("header", { children: [
        /* @__PURE__ */ jsxs("h4", { children: [
          "@zana-ai/mcp",
          info && /* @__PURE__ */ jsx("span", { className: `zana-ver-badge zana-ver-badge--${state.tone}`, children: state.badge })
        ] }),
        /* @__PURE__ */ jsx("p", { className: "settings-help", children: "Multi-agent orchestrator MCP server" })
      ] }),
      loading ? /* @__PURE__ */ jsx("p", { className: "settings-help", children: "Checking…" }) : failed ? /* @__PURE__ */ jsx("p", { className: "modal-error", children: "Couldn’t run the version check." }) : info ? /* @__PURE__ */ jsxs(Fragment, { children: [
        /* @__PURE__ */ jsxs("div", { className: "zana-ver-grid", children: [
          /* @__PURE__ */ jsx("span", { className: "zana-ver-key", children: "Installed" }),
          /* @__PURE__ */ jsx("span", { className: "zana-ver-val zana-ver-val--mono", children: info.installed ?? "—" }),
          /* @__PURE__ */ jsx("span", { className: "zana-ver-key", children: "Latest" }),
          /* @__PURE__ */ jsx("span", { className: "zana-ver-val zana-ver-val--mono", children: info.latest ?? "—" })
        ] }),
        /* @__PURE__ */ jsxs("p", { className: `zana-ver-status zana-ver-status--${state.tone}`, children: [
        ...
        (info.updateAvailable || !info.installed) && /* @__PURE__ */ jsxs("div", { className: "zana-ver-upgrade", children: [
          /* @__PURE__ */ jsx("code", { className: "settings-code-block zana-ver-cmd", children: info.upgradeCommand }),
          /* @__PURE__ */ jsxs("button", { type: "button", className: "settings-btn", onClick: copyUpgrade, children: [
            /* @__PURE__ */ jsx(Copy, { size: 14 }),
            copied ? "Copied" : "Copy"
          ] })
        ] })
      ] }) : null,
      /* @__PURE__ */ jsx("div", { className: "settings-btn-row", children: /* @__PURE__ */ jsxs("button", { type: "button", className: "settings-btn", onClick: load, disabled: loading, children: [
