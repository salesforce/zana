const m = "consensus-reanalysis", w = 50, l = "records", f = "seeded", y = {
  ...
  setup(r) {
    async function i() {
      ...
      return t.length === 0 && !await r.storage.get(f) ? (await r.storage.set(l, [y]), await r.storage.set(f, !0), [y]) : t;
    }
    async function g(t) {
      const e = (await i()).filter((a) => a.id !== t.id);
      e.push(t), await r.storage.set(l, e);
    }
    function v(t, n) {
      const e = t;
      if (!e || typeof e.question != "string" || typeof e.verdict != "string" || !Array.isArray(e.votes)) return null;
      const a = e.votes.filter((s) => s && typeof s.voter == "string" && typeof s.stance == "string").map((s) => ({ voter: s.voter, stance: s.stance, rationale: String(s.rationale ?? "") }));
      if (a.length === 0) return null;
      const u = typeof e.settledAt == "string" && e.settledAt ? e.settledAt : (/* @__PURE__ */ new Date()).toISOString();
      return {
        id: `reanalysis-${n.replace(/[^a-zA-Z0-9_-]/g, "-")}`,
        projectId: typeof e.projectId == "string" ? e.projectId : void 0,
        question: e.question,
        verdict: e.verdict,
        synthesis: typeof e.synthesis == "string" ? e.synthesis : void 0,
        votes: a,
        dissent: Array.isArray(e.dissent) ? e.dissent.filter((s) => typeof s == "string") : void 0,
        settledAt: u,
    ...
    return {
       ...
       */
      async list(t) {
        return (await i()).filter((e) => !t || e.projectId === t).slice().sort((e, a) => (a.settledAt ?? "").localeCompare(e.settledAt ?? "")).map((e) => ({
          id: e.id,
          question: e.question,
          verdict: e.verdict,
      ...
      async get(t) {
        return (await i()).find((e) => e.id === t) ?? null;
      },
      /**
      ...
      async record(t) {
        ...
        return await g(t), r.log(`recorded consensus ${t.id} (${t.verdict})`), { id: t.id };
      },
      /**
      ...
      async ingest(t) {
        ...
        let a;
        try {
          a = await n("zana", "zana_artifact_list", { tag: m }, e);
        } catch (s) {
          return r.log("ingest: zana_artifact_list failed (zana MCP unavailable?)", s), { ingested: 0 };
        }
        const u = (Array.isArray(a) ? a : []).filter((s) => !!s && typeof s.id == "string").slice(0, w);
        let p = 0;
        for (const s of u) {
          const c = s.id;
          try {
            const o = await n("zana", "zana_artifact_read", { artifactId: c }, e), h = o && typeof o == "object" ? o.content : void 0;
            if (typeof h == "string") {
              const d = v(JSON.parse(h), c);
              d && (await g(d), p++, r.log(`ingested re-analysis ${d.id} (${d.verdict})`));
            }
            await n("zana", "zana_artifact_delete", { artifactId: c }, e);
          } catch (o) {
            r.log(`ingest: skip artifact ${c}`, o);
          }
        }
        return { ingested: p };
      },
      ...
      async remove(t) {
        const n = await i(), e = n.filter((a) => a.id !== t);
        return await r.storage.set(l, e), r.log(`removed consensus ${t}`), { removed: t, ok: e.length !== n.length };
      },
      /**
       ...
       */
      async clearAll() {
        const n = (await i()).length;
        return await r.storage.set(l, []), await r.storage.set(f, !0), r.log(`cleared all consensus records (${n})`), { cleared: n };
      }
    };
  }
};
