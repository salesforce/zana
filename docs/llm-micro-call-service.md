...
3. **`LlmProvider`** — the transport seam (`src/main/llm/provider.ts`). One implementation per backend. **Contract: never throw** — every failure (spawn error, timeout, non-zero exit, missing provider, no key, HTTP error, network error, abort) resolves to an `LlmRunResult { ok: false, error }`, so callers stay branch-free.
...
| File | Role |
|---|---|
| `src/shared/types.ts` | `LlmProviderId`, `LlmPromptEntry`, `LlmRunResult`, `CcApi.llmPrompts` |
| `src/main/llm/provider.ts` | `LlmProvider` interface, `LlmRunRequest`, `LlmProviderMap` |
| `src/main/llm/claude-cli-provider.ts` | v1 transport — `claude --print` |
| `src/main/llm/openai-provider.ts` | OpenAI transport — HTTP chat-completions, API key |
| `src/main/llm/gemini-provider.ts` | Gemini transport — HTTP `generateContent`, API key |
| `src/main/llm/model-aliases.ts` | shared tier-alias map (`resolveModelAlias`) + config-write-time validation (`isKnownModel`) |
| `src/main/llm-service.ts` | `LlmService`, `fillTemplate`, `availableProviders`, `setProvider` |
| `src/main/prompt-registry.ts` | `PromptRegistry` + the `builtin:tab-namer` entry |
| `src/main/index.ts` | singletons (`promptRegistry`, `llmService`), the `rebuildProviders(config)` helper, IPC handlers, the tab-namer consumer |
| `src/main/voice/secrets.ts` | encrypted (`safeStorage`) OpenAI/Gemini API keys + env-var fallbacks |
| `src/renderer/components/settings/PromptsTab.tsx` | the editor UI |
...
`{{var}}` placeholders are filled from the `vars` map the caller passes to `run()`. Unknown placeholders are left literal (visible, not silently blank). Field reference: `LlmPromptEntry` in `src/shared/types.ts`.
...
```ts
// src/main/llm/anthropic-sdk-provider.ts
...
The `LlmProviderId` union in `src/shared/types.ts` already enumerates `'anthropic-sdk'`, and a prompt selects a provider via `"provider": "…"`. No change to `LlmService` or any consumer. **No refactor — one file + one `setProvider` line.** See [Other models](#other-models-multi-provider) for how keys and model-id mapping already work.
...
1. `src/shared/ipc.ts` — add `run: 'llmPrompts:run'` to the `llmPrompts` group.
...
3. `src/preload/index.ts` — `run: (id, vars) => ipcRenderer.invoke(IPC.llmPrompts.run, id, vars)`.
4. `src/shared/types.ts` — add `run(id, vars): Promise<LlmRunResult>` to `CcApi.llmPrompts`.
