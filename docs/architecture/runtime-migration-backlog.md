...
confinement, and existing user data. Plugins are full-trust in-process code on
the server (BB's model), not per-extension Electron utility processes.
...
- Plugins are full-trust TypeScript packages loaded **in-process on `apps/server`**
  after an explicit install/enable. They call `ZccPluginApi`; they never receive
  host-daemon tokens or signing keys. The server is the only signer of execution
  commands. Install uses path containment, `engines` gates, `npm --ignore-scripts`,
  and native-addon rejection — not capability tokens or a permission broker.
...
### 5. Plugin and Integration Services
...
1. Server-owned `PluginService`: `package.json` `zcc` manifests, install sources
   ...
   durable-store, in-process `loadOne` / `disposeOne`, builtin vs official
   registry, status/degraded, asset serving at `/plugins/:id/assets/`.
2. Renderer slot registry (`definePluginApp`): wholesale replace per plugin id,
   generation remount, per-slot error boundaries. Desktop does not spawn plugin
