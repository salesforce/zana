const titleSegmenter = new Intl.Segmenter('en', { granularity: 'grapheme' });

/**
 * Derive a short, meaningful tab title from an agent's opening instruction.
 * Collapses whitespace and caps at 40 graphemes with an ellipsis, so a launched
...
export function titleFromPrompt(prompt: string): string {
  ...
  if (!oneLine) return '';
  let count = 0;
  for (const segment of titleSegmenter.segment(oneLine)) {
    if (count++ === 40) return `${oneLine.slice(0, segment.index)}…`;
  }
  return oneLine;
}
