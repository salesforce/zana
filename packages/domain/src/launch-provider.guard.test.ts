 ...
 *
 * `packages/domain/src/launch-provider.ts` is the SINGLE source of truth for the launch
 ...
 *
 * This walks `src/main/**` + `apps/app/src/**` `*.ts` / `*.tsx`, STRIPS comments
 ...
 * `apps/app/src/__tests__/rule6-zana-literal.guard.test.ts`.
 */
...
const SHARED_MODULE = 'packages/domain/src/launch-provider.ts';
...
function findOffenders(): string[] {
  ...
  const roots = [
    ...
    join(REPO_ROOT, 'apps/host-daemon/src'),
    ...
    expect(
      ...
      offenders.length === 0
        ...
        : `Duplicated profile logic reappeared. Import from ` +
            `'@zana-ai/zcc-domain/launch-provider' (isClaudeProfile / VALID_PROFILES / parseProfile) ` +
