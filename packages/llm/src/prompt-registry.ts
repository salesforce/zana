...
import type { LlmPromptEntry, LlmProviderId } from '@zana-ai/zcc-domain/llm';
import { isKnownModel } from './model-aliases.js';

export interface PromptRegistryDeps {
  /** Absolute dir for user prompt JSON files. Overrides {@link homeDir}. */
  userDir?: string | (() => string);
  /** HOME used to derive `~/.zcc/llm-prompts` when `userDir` is omitted. */
  homeDir?: string | (() => string);
  /** Open the user dir in a file manager. Injected so this package stays Electron-free. */
...
export class PromptRegistry extends EventEmitter {
  ...
  constructor(private readonly deps: PromptRegistryDeps = {}) {
    super();
  }

  private userDir(): string {
    if (this.deps.userDir) return resolvePath(this.deps.userDir);
    const home = this.deps.homeDir ? resolvePath(this.deps.homeDir) : homedir();
    return join(home, '.zcc', 'llm-prompts');
  }
  ...
  refresh() {
    ...
    for (const p of listInDir(this.userDir())) merged.set(p.id, p);
  ...
  deleteUser(id: string): void {
    const file = join(this.userDir(), fileNameForId(id));
    try {
  ...
  async revealUserDir(): Promise<{ ok: boolean; path: string; message?: string }> {
    const path = this.userDir();
    try {
      ensureDir(path);
      ensureReadme(path);
      await (this.deps.revealPath ?? (async () => undefined))(path);
      return { ok: true, path };
    } catch (err) {
      return {
        ok: false,
