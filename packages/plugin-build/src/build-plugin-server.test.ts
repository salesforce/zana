...
describe("plugin server build", () => {
  ...
  it("keeps both the current and the pre-rename SDK specifier external", () => {
    expect(PLUGIN_SERVER_EXTERNALS).toContain("@zana-ai/zcc-plugin-sdk");
  ...
  it("builds a pre-rename source importing bare @bb/plugin-sdk", async () => {
    ...
    await writeFile(
      join(dir, "server.ts"),
      [
        'import type { ZccPluginApi } from "@bb/plugin-sdk";',
  ...
  describe("SDK subpath imports", () => {
    ...
    const serverSource = [
      'import { defineRpcContract, type ZccPluginApi } from "@zana-ai/zcc-plugin-sdk";',
    ...
    it("keeps the bare specifier external and bundles the subpath from the plugin's SDK", async () => {
      ...
      const bundle = await readFile(jsPath, "utf8");
      expect(bundle).toContain('from "@zana-ai/zcc-plugin-sdk"');
      expect(bundle).not.toContain('"@zana-ai/zcc-plugin-sdk/host"');
