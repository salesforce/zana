...
function useSlotEnv(hook: string): SlotEnv {
  ...
  if (!env) {
    throw new Error(
      `${hook}() needs the test slot environment — mount the component via renderSlot(...) from @zana-ai/zcc-plugin-sdk/testing/app`,
    );
  }
  return env;
}
 ...
 * (and therefore `@zana-ai/zcc-plugin-sdk/app`) is imported.
 */
...
export async function loadPluginApp(
  ...
  if (!isPluginAppDefinition(definition)) {
    throw new Error(
      "the bundle's default export is not definePluginApp(...) from @zana-ai/zcc-plugin-sdk/app",
    );
  }
