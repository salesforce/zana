...
/** SQLite handle surface used by plugin storage. Host-owned; not a driver import. */
...
/** HTTP handler context. Host supplies a Hono-compatible request object. */
export type PluginHttpContext = Record<string, unknown>;
import type {
  Environment,
  Host,
  PermissionMode,
...
/** Product HTTP/realtime client. ZCC wires this to the control-plane client. */
export type BbSdk = Record<string, unknown>;
...
export interface PluginStorage {
   ...
   */
  migrate(db: PluginSqliteDatabase, statements: string[]): void;
}
 ...
 * `ProviderStrings` in `@zana-ai/zcc-domain/thread-runtime`, which is the client projection.
 */
