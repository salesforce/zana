...
const handshake: NativeShellHandshake = {
  ...
  capabilities: ['haptic', 'badge', 'share', 'open-external', 'safe-area', 'open-native']
};
...
describe('parsePageToShellMessage', () => {
  it('accepts every message kind the contract defines', () => {
    const cases: unknown[] = [
      ...
      { type: 'title', title: 'zccMobile', path: '/' },
      ...
      { type: 'open-native', screen: 'device-settings' },
      {
        ...
        request: {
          kind: 'share',
          payload: { url: 'https://phone.example/threads/thr_1' }
        }
      }
    ];
  ...
  it('drops a message type an older shell does not know', () => {
    const parsed = parsePageToShellMessage(json({ type: 'open-camera', lens: 'front' }));
    expect(parsed.ok).toBe(false);
  });

  it('refuses a native screen the shell does not own', () => {
    expect(
      parsePageToShellMessage(json({ type: 'open-native', screen: '/settings/servers' })).ok
    ).toBe(false);
    expect(parsePageToShellMessage(json({ type: 'open-native', screen: '' })).ok).toBe(false);
  });
  ...
  it('rejects a badge count that is negative or fractional', () => {
    expect(parsePageToShellMessage(json({ type: 'badge', count: -1 })).ok).toBe(false);
    expect(parsePageToShellMessage(json({ type: 'badge', count: 1.5 })).ok).toBe(false);
  });
  ...
  it('refuses any external link that is not http or https', () => {
    for (const url of [
      ...
      expect(parsePageToShellMessage(json({ type: 'open-external', url })).ok, url).toBe(false);
    }
    expect(
      parsePageToShellMessage(json({ type: 'open-external', url: 'https://example.com/docs' })).ok
    ).toBe(true);
  });
    ...
    expect(
      parsePageToShellMessage(
        json({
          type: 'request',
          id: 'r1',
          request: { kind: 'share', payload: { title: 'zccMobile' } }
        })
      ).ok
    ).toBe(false);
  });
...
describe('parseNativeShellHandshake', () => {
  ...
  it('reads a malformed or absent bridge as no bridge', () => {
    ...
    expect(parseNativeShellHandshake({ ...handshake, safeArea: { top: 1 } })).toBeNull();
    expect(parseNativeShellHandshake({ ...handshake, capabilities: ['teleport'] })).toBeNull();
  });
});
