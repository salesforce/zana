...
export function createThread(
  ...
  db.sqlite.prepare(
    `INSERT INTO threads (id, project_id, host_id, environment_id, provider_id, status, title, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    id,
    input.projectId,
...
export function getThread(db: ZccDatabase, id: string): ThreadRow | null {
  const row = db.sqlite.prepare('SELECT * FROM threads WHERE id = ?').get(id) as ThreadSqlRow | undefined;
...
export function listThreadsByHost(db: ZccDatabase, hostId: string): ThreadRow[] {
  return (db.sqlite.prepare('SELECT * FROM threads WHERE host_id = ?').all(hostId) as ThreadSqlRow[]).map(toThread);
}

export function listLiveThreads(db: ZccDatabase): ThreadRow[] {
  return (db.sqlite.prepare(
    `SELECT * FROM threads WHERE status IN ('starting', 'running') ORDER BY updated_at DESC`
...
export function disconnectLiveThreadsForHost(db: ZccDatabase, hostId: string): number {
  ...
  const result = db.sqlite.prepare(
    `UPDATE threads SET status = 'disconnected', updated_at = ?
