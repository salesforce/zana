...
function resolveFakeProviderScriptPath(): string {
  ...
  const candidates = [
    fileURLToPath(new URL("./fake-provider-script.ts", import.meta.url)),
    join(here, "..", "..", "src", "test", "fake-provider-script.ts"),
  ];
  let dir = here;
  for (let i = 0; i < 10; i += 1) {
    candidates.push(join(dir, "packages/agent-runtime/src/test/fake-provider-script.ts"));
    dir = dirname(dir);
  }
...
export const fakeProviderScriptPath = (() => {
  try {
    return resolveFakeProviderScriptPath();
  } catch {
    return "";
  }
})();
