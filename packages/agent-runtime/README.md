...
```typescript
import { createAgentRuntime } from "@zana-ai/zcc-agent-runtime";
...
pnpm --filter @zana-ai/zcc-agent-runtime test:unit

# Integration tests (requires real provider credentials)
pnpm --filter @zana-ai/zcc-agent-runtime test:integration

# All tests
pnpm --filter @zana-ai/zcc-agent-runtime test
```
...
```bash
pnpm --filter @zana-ai/zcc-agent-runtime test:integration -- --reporter=verbose > /tmp/integ-out.txt 2>&1
# Then inspect:
...
`@zana-ai/zcc-agent-runtime` is source-only inside this workspace. The host daemon build
