...
describe("formatAcpLaunchFailure", () => {
  it("maps spawn ENOENT to a missing-on-this-machine message", () => {
    ...
    expect(formatAcpLaunchFailure("/usr/bin/cursor-agent", missing)).toContain(
      "Cursor is not installed on this machine",
    );
    expect(formatAcpLaunchFailure("opencode", new Error("exited 1"))).toBe(
      'Failed to launch ACP agent "opencode": exited 1',
    );
    expect(formatAcpLaunchFailure("opencode", new Error("spawn opencode ENOENT"))).toContain(
